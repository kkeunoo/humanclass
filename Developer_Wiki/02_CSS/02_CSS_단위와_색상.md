...
| 문서 | `02_CSS_단위와_색상.md` |
| 분류 | `02_CSS` |
| 권장 선수 학습 | `01_CSS_선택자와_적용방법.md` |
| 다음 학습 | `03_CSS_박스모델.md` |
...
> 이 문서는 수업 원본의 CSS 단위 실습을 기준으로 구성했습니다. 원본에 직접 등장하지 않는 `ch`, `dvh`, `clamp()`, CSS 사용자 지정 속성 등은 실무 확장 학습으로 구분해 추가했습니다.
...
```text
root em의 약자로 body의 크기를 활용한다
```

강사님 코드 주석:

```text
rem : root em, body의 크기를 활용한다
```
...
## Q1. `em`과 `rem`의 차이는 무엇인가요?

`em`은 현재 요소 또는 부모의 글자 크기를 기준으로 계산되어 중첩 시 누적될 수 있습니다. `rem`은 루트 `html`의 글자 크기를 기준으로 합니다.

## Q2. `font-size: 150%`는 무엇을 기준으로 하나요?

부모 요소의 계산된 `font-size`를 기준으로 합니다.

## Q3. `100vw`와 `100%`는 어떻게 다른가요?

`100vw`는 뷰포트 너비, `100%`는 해당 요소의 containing block을 기준으로 합니다.

## Q4. `background-color`는 상속되나요?

일반적으로 상속되지 않습니다. 자식 배경이 투명하여 부모 배경이 뒤에서 보일 수 있습니다.

## Q5. `rgba()`와 `opacity`의 차이는 무엇인가요?

RGBA의 알파는 해당 색상만 투명하게 만들고, `opacity`는 요소 전체와 자식 콘텐츠까지 투명하게 만듭니다.

## Q6. `#0fa`는 어떤 색상과 같은가요?

`#00ffaa`와 같습니다.

## Q7. 8자리 HEX의 마지막 두 자리는 무엇인가요?

알파 채널입니다.

## Q8. `vmin`과 `vmax`의 차이는 무엇인가요?

`vmin`은 뷰포트 너비와 높이 중 작은 값, `vmax`는 큰 값을 기준으로 합니다.

## Q9. 모바일에서 `100vh`가 문제가 될 수 있는 이유는 무엇인가요?

브라우저 주소창과 도구 모음이 나타나고 사라지며 실제 보이는 영역이 바뀔 수 있기 때문입니다.

## Q10. 글자 크기에 `rem`을 자주 사용하는 이유는 무엇인가요?

부모 중첩에 의한 누적을 피하면서 루트 글자 크기를 기준으로 일관된 크기 체계를 만들 수 있기 때문입니다.
