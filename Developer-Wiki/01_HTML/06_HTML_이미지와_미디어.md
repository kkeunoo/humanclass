...
- [학습 목표](#html-1)
- [개념에서 실제 동작까지](#html-2)
- [1. 웹에서 이미지와 미디어의 역할](#html-3)
- [2. `img`: 이미지 삽입](#html-4)
- [3. `src`: 이미지 파일 경로](#html-5)
- [4. 이미지 경로 작성](#html-6)
- [5. `alt`: 대체 텍스트](#html-7)
- [6. 좋은 `alt` 작성법](#html-8)
- [7. 장식용 이미지의 `alt`](#html-9)
- [8. 기능을 가진 이미지의 `alt`](#html-10)
- [9. `title`과 `alt`의 차이](#html-11)
- [10. 이미지 크기 지정](#html-12)
- [11. 원본 비율과 이미지 왜곡](#html-13)
- [12. 한쪽 크기만 지정하기](#html-14)
- [13. HTML 속성과 CSS의 역할](#html-15)
- [14. 반응형 이미지 기본 설정](#html-16)
- [15. `object-fit`으로 이미지 영역 맞추기](#html-17)
- [16. `figure`와 `figcaption`](#html-18)
- [17. `alt`와 `figcaption`의 차이](#html-19)
- [18. `picture`: 화면 조건에 따라 이미지 선택](#html-20)
- [19. 이미지 형식에 따른 선택](#html-21)
- [20. `srcset`과 해상도 대응](#html-22)
- [21. 이미지 지연 로딩](#html-23)
- [22. 이미지 성능 최적화](#html-24)
- [23. `video`: 동영상 삽입](#html-25)
- [24. `video` 주요 속성](#html-26)
- [25. `source`: 여러 영상 형식 제공](#html-27)
- [26. 자동 재생 주의점](#html-28)
- [27. `poster`: 영상 대표 이미지](#html-29)
- [28. 영상 자막](#html-30)
- [29. `audio`: 오디오 삽입](#html-31)
- [30. `iframe`: 외부 문서 삽입](#html-32)
- [31. 유튜브 영상 삽입](#html-33)
- [32. 반응형 `iframe`](#html-34)
- [33. `iframe` 사용 시 주의점](#html-35)
- [34. `br`로 미디어 간격 만들기](#html-36)
- [35. 여러 속성을 줄바꿈하여 작성하기](#html-37)
- [36. 원본 실습 코드 분석](#html-38)
- [37. 내 코드와 강사님 코드 비교](#html-39)
- [38. 개선된 전체 예제](#html-40)
- [39. 실무 예제: 프로필 이미지](#html-41)
- [40. 실무 예제: 상품 카드](#html-42)
- [41. 실무 예제: 포트폴리오 프로젝트](#html-43)
- [42. 실무 예제: 반응형 배너](#html-44)
- [43. 실무 예제: 강의 영상](#html-45)
- [44. 자주 하는 실수](#html-46)
- [45. 디버깅 체크리스트](#html-47)
- [46. 접근성 체크리스트](#html-48)
- [47. 성능 체크리스트](#html-49)
- [48. 종합실습](#html-50)
- [49. 정답과 해설](#html-51)
- [50. 최종 체크리스트](#html-52)
- [51. 핵심 요약](#html-53)
- [브라우저 해석 복습 카드 — 자원 URL 요청과 대체 콘텐츠](#html-54)
...
### 먼저 이해하기: HTML 수신과 이미지 수신은 별개다

img는 이미지 파일을 문서에 연결하는 요소입니다. src라는 문자열을 화면에 그리는 것이 아니라 URL을 계산하고 자원을 가져온 뒤 해석해 표시합니다. 캐시를 사용하면 네트워크에서 새로 내려받지 않을 수도 있으므로 반드시 임시 폴더에 특정 라이브러리가 저장한다고 외우지 않습니다.
...
| 순서 | 확인 대상 | 실패 때 점검 |
| --- | --- | --- |
| src URL 계산 | 현재 문서 폴더와 asset 위치 | 잘못된 상대 경로 |
| 자원 가져오기 | Network 상태와 Content-Type | 404·차단·연결 실패 |
| 이미지 해석 | 파일 형식과 실제 내용 | HTML 오류 페이지를 이미지로 받은 경우 |
| 크기·배치·표시 | CSS와 width/height | 숨김·0 크기·잘린 영역 |

alt는 의미를 대신 전달하는 텍스트입니다. “이미지가 없습니다”보다 무엇을 보여주려 했는지를 적습니다. 로딩 실패 화면의 대체 텍스트 표현은 브라우저마다 다를 수 있으며, 접근성에서의 목적과 시각적 모양을 구분합니다.

두 파일의 다음 이미지는 width="200" height="300"입니다. 원본 비율이 2:3이 아니라면 왜곡될 수 있습니다. 한쪽 크기만 지정하거나 CSS height: auto를 쓰는 보충 예제를 비교합니다. 실제 원본 크기를 확인하지 않고 “500×500 원본”이라고 단정하지 않습니다.

내 파일에는 iframe 예제가 주석 처리되어 있고 강사님 파일에는 없습니다. 주석 상태에서는 iframe 문서를 로드하지 않습니다. iframe은 문서 안의 별도 탐색 문맥이며 운영체제 수준의 브라우저 앱을 새로 실행하는 것은 아닙니다.

**직접 확인:** 로컬 이미지 URL 한 글자를 일부러 틀려 Network 결과를 비교한 뒤 되돌립니다. video의 controls를 제거하면 내장 재생 UI가 달라집니다. loop는 끝나면 반복하는 옵션이며 자동 재생을 시작하는 옵션이 아닙니다.

💡 알아두면 좋은 점: 외부 이미지·영상은 제공자 정책, 만료, 네트워크 상태에 영향을 받습니다. 성공을 보장하는 결과 예제가 아니라 요청 흐름 확인 자료로 사용합니다.


### 개발자 도구에서 값을 읽고 확인하기

06_img.html을 실제 브라우저에서 연 상태에서 개발자 도구의 Console에 다음을 입력합니다. 아래 코드는 HTML에 삽입할 태그가 아니라 브라우저의 JavaScript 확인 명령입니다. 💡 DOM·폼 API를 이용한 확인법으로, JavaScript 과정에서 더 자세히 배웁니다.

```javascript
document.querySelector('img').getAttribute('src')
```

**예상 결과:** asset/Spongebob-Christmas-PNG-Picture.png

getAttribute는 적은 속성 문자열을 읽습니다. document.querySelector('img').src는 계산된 URL을 읽으므로 둘의 표시값이 다를 수 있습니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
...
## 브라우저 해석 복습 카드 — 자원 URL 요청과 대체 콘텐츠
