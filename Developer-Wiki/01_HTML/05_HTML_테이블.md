...
- [개념에서 실제 동작까지](#html-2)
- [1. 테이블이란](#html-3)
- [2. 테이블을 사용해야 하는 경우](#html-4)
- [3. 테이블을 사용하면 안 되는 경우](#html-5)
- [4. 테이블의 기본 구성](#html-6)
- [5. `table`: 표 전체 영역](#html-7)
- [6. `tr`: 한 행](#html-8)
- [7. `td`: 데이터 셀](#html-9)
- [8. `th`: 제목 셀](#html-10)
- [9. `th`와 `td`의 차이](#html-11)
- [10. 행마다 셀 개수를 맞추기](#html-12)
- [11. 표 제목 `caption`](#html-13)
- [12. 표의 구조 구분](#html-14)
- [13. `thead`: 제목 영역](#html-15)
- [14. `tbody`: 데이터 영역](#html-16)
- [15. `tfoot`: 요약 영역](#html-17)
- [16. 셀 병합 `colspan`](#html-18)
- [17. 셀 병합 `rowspan`](#html-19)
- [18. `colspan`과 `rowspan` 함께 사용하기](#html-20)
- [19. 셀 안에 작성할 수 있는 콘텐츠](#html-21)
- [20. 원본 코드에서 확인한 오류와 개선점](#html-22)
- [21. 내 코드와 강사님 코드 비교](#html-23)
- [22. 대표 실무 예제: 게시판 목록](#html-24)
- [23. 반응형 테이블](#html-25)
- [24. 테이블 CSS 기초](#html-26)
- [25. `colgroup`과 `col`](#html-27)
- [26. 접근성 체크](#html-28)
- [27. 자주 하는 실수](#html-29)
- [28. 실무 개선 원칙](#html-30)
- [29. 종합실습](#html-31)
...
- [31. 최종 체크리스트](#html-33)
- [32. 핵심 요약](#html-34)
- [브라우저 해석 복습 카드 — 행·열 관계를 접근 가능한 표로 구성](#html-35)
...
### 먼저 이해하기: 표 병합은 격자 자리를 차지하는 일이다

표는 행과 열의 관계가 있는 데이터를 표현합니다. tr은 행, td는 데이터 칸, th는 제목 칸입니다. th가 굵게 보이는 것은 기본 스타일이고 제목 역할이 본질입니다. 행별 태그 개수보다 병합까지 포함한 격자 점유를 세어야 합니다.
...
| 격자 | 1열 | 2열 | 3열 |
| --- | --- | --- | --- |
| 1행 | 1-1의 시작 | 1-2의 시작 | 1-2가 차지 |
| 2행 | 1-1이 계속 차지 | 2-2 | 2-3 |

첫 행과 둘째 행에 td 태그가 각각 두 개 있어도 격자는 세 열입니다. 병합하지 않은 셀을 그대로 남기면 이미 점유된 자리 때문에 의도와 다른 배치가 됩니다. 따라서 먼저 격자를 그리고 병합 영역의 중복 셀을 제거합니다.

원본 차이도 중요합니다. 내 파일의 caption에는 hidden이 있고 강사님 caption에는 없습니다. hidden은 보통 시각 표시뿐 아니라 접근성 노출도 막으므로 “화면에는 숨기지만 스크린 리더는 항상 읽는다”는 의미가 아닙니다. 두 파일 모두 table 안에 summary를 넣었지만 summary는 표 설명 요소가 아닙니다. 표 제목은 caption, 자세한 설명은 표 앞 문단 등으로 제공합니다. 내 게시판 표에는 th를 td 종료 태그로 닫는 오타도 있습니다.

💡 개선 예제 — 원본 제목 셀에 열 관계 추가:
...
scope는 제목과 데이터의 관계를 명확하게 하는 보충 속성입니다. border="1" 실습은 원본 이해를 위해 남기지만 새 디자인의 선·간격은 CSS로 지정합니다.

**직접 확인:** Elements에서 tbody가 자동 삽입되는지 봅니다. 소스에 없던 tbody가 보이는 것은 브라우저의 파싱 결과일 수 있습니다. 표 밖으로 글자가 나가면 CSS부터 바꾸지 말고 table/tr 밖에 잘못 쓴 텍스트와 종료 태그를 먼저 확인합니다.


### 개발자 도구에서 값을 읽고 확인하기

05_table.html을 실제 브라우저에서 연 상태에서 개발자 도구의 Console에 다음을 입력합니다. 아래 코드는 HTML에 삽입할 태그가 아니라 브라우저의 JavaScript 확인 명령입니다. 💡 DOM·폼 API를 이용한 확인법으로, JavaScript 과정에서 더 자세히 배웁니다.
...
첫 표의 행 개수입니다. 브라우저가 tbody를 자동 추가해도 table.rows는 행을 찾아줍니다. 병합된 격자 개수와 td 태그 개수는 별개입니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
...
## 브라우저 해석 복습 카드 — 행·열 관계를 접근 가능한 표로 구성
