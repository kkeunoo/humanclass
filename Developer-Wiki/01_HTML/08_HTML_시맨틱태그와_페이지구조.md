...
- [개념에서 실제 동작까지](#html-2)
- [1. 시맨틱 HTML이란?](#html-3)
- [2. 모양과 의미는 다르다](#html-4)
- [3. 시맨틱 요소를 사용하는 이유](#html-5)
- [4. `div`는 잘못된 요소인가?](#html-6)
- [5. 페이지의 기본 골격](#html-7)
- [6. `header`](#html-8)
- [7. `nav`](#html-9)
- [8. 여러 개의 `nav`](#html-10)
- [9. `main`](#html-11)
- [10. `section`](#html-12)
- [11. `section`과 제목](#html-13)
- [12. `article`](#html-14)
- [13. `section`과 `article` 비교](#html-15)
- [14. `aside`](#html-16)
- [15. `footer`](#html-17)
- [16. 같은 태그를 여러 번 사용해도 되는가?](#html-18)
- [17. 제목 태그와 문서 구조](#html-19)
- [18. `h1` 사용 기준](#html-20)
- [19. 제목을 크기 때문에 선택하지 않는다](#html-21)
- [20. 랜드마크란?](#html-22)
- [21. `aria-label`은 언제 사용하는가?](#html-23)
- [22. 시맨틱 요소와 클래스는 함께 사용할 수 있다](#html-24)
- [23. 나쁜 예: `div`만 사용하는 페이지](#html-25)
- [24. 개선 예](#html-26)
- [25. 블로그 페이지 구조](#html-27)
- [26. 랜딩 페이지 구조](#html-28)
- [27. 포트폴리오 페이지 구조](#html-29)
- [28. 뉴스 목록 구조](#html-30)
- [29. 관리자 화면 구조](#html-31)
- [30. `div`와 `section` 선택 기준](#html-32)
- [31. 시맨틱 태그를 과도하게 사용하지 않는다](#html-33)
- [32. 잘못된 중첩 예](#html-34)
- [33. 링크와 버튼의 의미 구분](#html-35)
- [34. 목록과 내비게이션](#html-36)
- [35. 이미지와 시맨틱 구조](#html-37)
- [36. 폼과 시맨틱 구조](#html-38)
- [37. 표와 시맨틱 구조](#html-39)
- [38. 원본 수업 코드와의 연결](#html-40)
- [39. 통합 프로젝트 예제](#html-41)
- [40. 구조 설계 순서](#html-42)
- [41. 자주 하는 실수](#html-43)
- [42. 디버깅과 검수 방법](#html-44)
- [43. 접근성 체크리스트](#html-45)
- [44. 실무 리뷰 기준](#html-46)
- [45. 종합실습](#html-47)
- [46. 정답과 해설](#html-48)
- [47. 최종 체크리스트](#html-49)
- [48. 핵심 요약](#html-50)
- [브라우저 해석 복습 카드 — 콘텐츠 역할을 DOM과 접근성 트리에 전달](#html-51)
...
### 먼저 이해하기: 시맨틱 구조는 내용의 역할을 전달한다

시맨틱은 의미가 있다는 뜻입니다. header·nav·main·article 등으로 페이지 부분의 역할을 표현합니다. 기본 모양이 비슷해도 div와 nav가 같은 의미를 갖는 것은 아닙니다. CSS를 끈 상태에서도 구조를 이해할 수 있도록 만드는 이유입니다.

**수업과 보충의 경계:** 번호형 HTML 수업 소스는 01_hello부터 07_form까지입니다. 이 문서의 페이지 설계 예제를 그 소스에서 그대로 사용했다고 소개하지 않습니다. 제목·링크·목록·표·폼을 연결하는 💡 확장 학습으로 읽습니다. 기존 실습 페이지와 연결한 설명도 원본에 있는 요소와 새로 제안한 구조를 구분합니다.

💡 재현 예제:

```html
<header><h1>나의 학습 기록</h1></header>
<nav aria-label="주요 메뉴"><a href="#study">학습 내용</a></nav>
<main id="study">
  <section>
    <h2>HTML 학습</h2>
    <p>제목과 문단을 구분했습니다.</p>
...
화면에는 제목·링크·소제목·문단이 보입니다. nav를 썼다고 메뉴가 가로로 정렬되는 것은 아닙니다. role이 있는 영역을 DOM과 접근성 정보로 전달하고, 보이는 배치는 CSS로 정합니다. aria-label은 영역 식별을 돕는 보충이며 화면에 메뉴 이름 글자를 자동 추가하지 않습니다.

article은 독립적으로 배포·재사용할 수 있는 내용, section은 관련 주제의 묶음에 적합합니다. 단순한 디자인 래퍼라면 div를 선택합니다. 모든 div를 section으로 바꾸는 것은 구조 개선이 아닙니다.

**처리 순서:** 콘텐츠 분류→제목 계층 결정→역할에 맞는 영역 선택→링크와 입력의 이름 확인→DOM·접근성 구조 확인→디자인 적용. 디자인부터 보고 제목 등급을 고르지 않습니다.

**직접 확인:** CSS 없이도 제목의 순서와 메뉴 목적이 이해되는지 봅니다. 키보드 Tab으로 링크와 입력 요소를 이동하고 화면에 보이는 순서와 크게 어긋나지 않는지 확인합니다. 스크린 리더를 실제로 쓰지 않았다면 스크린 리더 검수까지 완료했다고 기록하지 않습니다.


### 개발자 도구에서 값을 읽고 확인하기

이 문서의 시맨틱 재현 예제을 실제 브라우저에서 연 상태에서 개발자 도구의 Console에 다음을 입력합니다. 아래 코드는 HTML에 삽입할 태그가 아니라 브라우저의 JavaScript 확인 명령입니다. 💡 DOM·폼 API를 이용한 확인법으로, JavaScript 과정에서 더 자세히 배웁니다.
...
예제 코드를 실제 페이지에 넣었을 때의 결과입니다. aria-label을 읽는 것은 접근성 이름 검수의 일부이며 스크린 리더 실제 테스트를 대체하지 않습니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
...
## 브라우저 해석 복습 카드 — 콘텐츠 역할을 DOM과 접근성 트리에 전달
