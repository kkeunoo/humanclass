...
- [1. 폼이란?](#html-3)
- [2. `form` 요소](#html-4)
- [3. 폼 데이터의 기본 구조](#html-5)
- [4. `input` 기본 사용법](#html-6)
- [5. `type="text"`](#html-7)
- [6. `value`: 초기값과 전송값](#html-8)
- [7. `placeholder`](#html-9)
- [8. `readonly`와 `disabled`](#html-10)
- [9. `type="password"`](#html-11)
- [10. 버튼의 종류](#html-12)
- [11. 잘못 작성한 속성은 원하는 기능을 하지 않는다](#html-13)
- [12. 체크박스](#html-14)
- [13. 라디오 버튼](#html-15)
- [14. `label`](#html-16)
- [15. `select`와 `option`](#html-17)
- [16. 다중 선택 `select`](#html-18)
- [17. `textarea`](#html-19)
- [18. 폼 초기화](#html-20)
- [19. 다양한 입력 타입](#html-21)
- [20. 실무에서 자주 사용하는 추가 입력 타입](#html-22)
- [21. `required`와 기본 검증](#html-23)
- [22. `action`](#html-24)
- [23. `method`](#html-25)
- [24. GET 방식](#html-26)
- [25. POST 방식](#html-27)
- [26. GET과 POST 비교](#html-28)
- [27. Live Server에서 POST 오류가 발생하는 이유](#html-29)
- [28. 검색 폼 실습](#html-30)
- [29. 내 코드 분석](#html-31)
- [30. 내 코드에서 확인된 개선점](#html-32)
- [31. 강사님 코드 분석](#html-33)
- [32. 내 코드와 강사님 코드 비교](#html-34)
- [33. 강사님 코드에서 주의해서 볼 부분](#html-35)
- [34. 권장 통합 예제: 회원가입 폼](#html-36)
- [35. `fieldset`과 `legend`](#html-37)
- [36. 폼 접근성 체크리스트](#html-38)
- [37. 폼 보안과 서버 검증](#html-39)
- [38. 자주 하는 실수](#html-40)
- [39. 디버깅 방법](#html-41)
- [40. 실무 개선 예제](#html-42)
- [41. 종합실습](#html-43)
...
- [43. 최종 체크리스트](#html-45)
- [44. 핵심 요약](#html-46)
- [브라우저 해석 복습 카드 — 사용자 입력에서 HTTP 요청 데이터까지](#html-47)
...
### 먼저 이해하기: 화면 입력에서 HTTP 요청까지

폼은 입력 요소를 묶고 전송 방법과 목적지를 정하는 구조입니다. name은 전송할 키, 입력한 현재 값은 전송 값입니다. id는 label 연결 등에 쓰이며 name을 대신하지 않습니다. 실제 저장·검색·가입은 요청을 받은 서버가 처리합니다. HTML만으로 데이터베이스 저장이 완성되지 않습니다.

두 파일 공통 발췌 — GET 실습의 핵심:
...
<form method="get">
    ...
    <input type="submit" value="get 연습">
</form>
```

현재 URL이 http://127.0.0.1:5500/07_form.html이고 첫 칸에 hype, 둘째 칸에 memo, 셋째 칸에 artist를 입력한다고 가정합니다.

1. 제출 버튼을 누르면 폼의 기본 검증을 수행합니다.
2. 전송 대상 요소에서 이름-값 항목을 구성합니다.
3. 둘째 칸에는 name이 없어 전송 항목에 포함되지 않습니다.
4. GET 방식으로 song=hype&singer=artist를 URL 쿼리에 넣습니다.
5. 서버가 해당 URL 요청을 받아 응답합니다. 입력란을 만들었다고 서버 검색 기능이 생기는 것은 아닙니다.

예상 목적지:

```text
http://127.0.0.1:5500/07_form.html?song=hype&singer=artist
```

**readonly를 정확하게 읽기:** 두 원본의 readonly 칸에는 name이 없습니다. readonly 자체는 이름 있는 정상 전송 요소를 제외하지 않지만, 이 원본 칸은 name이 없어서 전송되지 않습니다. disabled 칸은 전송 대상에서 제외됩니다. 다음은 두 조건을 구분하는 재현 예제입니다.

```html
<input name="account" value="learner" readonly>
<input name="secret" value="hidden" disabled>
```

이 두 요소만 가진 GET 폼이라면 account=learner는 들어가고 secret 항목은 없습니다. “readonly면 무조건 서버로 간다” 대신 name과 전체 전송 조건을 함께 확인합니다.

내 체크박스 얼음추가는 value="5"이고 강사님 간얼음에는 value가 없습니다. 체크했을 때 내 코드는 coffee=5, 강사님 코드는 기본값으로 coffee=on을 보냅니다. 체크하지 않으면 둘 다 해당 항목을 보내지 않습니다. 강사님 gender에는 checked가 두 개지만 내 코드에는 여자 하나만 있습니다. 같은 그룹에 초기 선택은 하나만 지정합니다.

POST는 기본 폼 인코딩에서 데이터를 요청 본문에 담습니다. 주소창 쿼리에 안 보인다는 뜻이지 암호화가 아닙니다. POST 후에도 서버의 redirect 등으로 주소가 달라질 수 있으므로 “주소는 절대 바뀌지 않는다”는 규칙이 아닙니다.

**직접 확인:** Network에서 제출 요청의 URL·Method·Payload·응답 상태를 확인합니다. 정적 개발 서버가 POST를 처리하지 못해 오류를 반환해도 요청 본문이 만들어졌을 수 있습니다. 서버 종류와 설정에 따라 응답 코드는 달라질 수 있습니다. 반복 name은 여러 항목이며 보통 객체로 단순 변환하면 하나가 덮어써질 수 있습니다.

이 원리의 근거는 [HTML 표준의 폼 소개](https://html.spec.whatwg.org/multipage/forms.html)입니다.


### 개발자 도구에서 값을 읽고 확인하기

07_form.html을 실제 브라우저에서 연 상태에서 개발자 도구의 Console에 다음을 입력합니다. 아래 코드는 HTML에 삽입할 태그가 아니라 브라우저의 JavaScript 확인 명령입니다. 💡 DOM·폼 API를 이용한 확인법으로, JavaScript 과정에서 더 자세히 배웁니다.

```javascript
Array.from(new FormData(document.querySelector('input[name="song"]').form).entries())
```

**예상 결과:** [['song', 'hype'], ['singer', 'artist']]

song 칸에 hype, 이름 없는 칸에 memo, singer 칸에 artist를 먼저 입력합니다. 이름 없는 칸이 빠지는 것을 확인합니다. 이 코드는 제출 요청을 보내지 않고 전송 항목만 구성하므로 실제 Network 검사는 제출 버튼으로 따로 합니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
...
사용자가 값을 수정할 수 없지만, name이 있고 다른 제출 조건을 만족하면 폼 전송에 포함됩니다. 바로 아래 개선 예제에는 name이 있지만 원본의 readonly 칸에는 name이 없습니다.
...
`name`이 있고 다른 제출 조건을 만족하는 `readonly` 입력은 전송되며, `disabled` 입력은 전송 대상에서 제외됩니다. 원본의 이름 없는 readonly 칸은 name이 없어 전송되지 않습니다.
...
- 이름이 있는 readonly 입력은 다른 전송 조건을 만족하면 제출되지만 disabled 입력은 제외된다. 원본처럼 name이 없으면 readonly라도 전송되지 않는다.
...
## 브라우저 해석 복습 카드 — 사용자 입력에서 HTTP 요청 데이터까지
