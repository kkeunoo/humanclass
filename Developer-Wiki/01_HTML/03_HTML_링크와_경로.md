...
- [학습 목표](#html-1)
- [개념에서 실제 동작까지](#html-2)
- [1. 링크를 만드는 `a` 요소](#html-3)
- [2. `href`가 없는 `a` 요소](#html-4)
- [3. 현재 탭과 새 탭](#html-5)
- [4. URL 스킴](#html-6)
- [5. 이미지 링크](#html-7)
- [6. 경로를 이해하기 위한 폴더 구조](#html-8)
- [7. 상대 경로](#html-9)
- [8. 절대 URL](#html-10)
- [9. 루트 상대 경로](#html-11)
- [10. 경로 방식 비교](#html-12)
- [11. 문서 내부 이동](#html-13)
- [12. 반복 `br`로 거리를 만들지 않는다](#html-14)
- [13. 파일 다운로드 링크](#html-15)
- [14. 링크 접근성](#html-16)
- [15. 내 코드와 강사님 코드 비교](#html-17)
- [16. 개선된 통합 예제](#html-18)
- [17. 자주 하는 실수](#html-19)
- [18. 실무 팁](#html-20)
- [19. 종합실습](#html-21)
- [20. 정답과 해설](#html-22)
- [21. 최종 체크리스트](#html-23)
- [22. 핵심 요약](#html-24)
- [브라우저 해석 복습 카드 — 현재 문서 URL에서 목적 URL 계산](#html-25)
...
### 먼저 이해하기: 링크의 입력은 현재 URL과 href다

링크를 배우는 이유는 파일 위치와 서버가 제공하는 URL을 구분하기 위해서입니다. 브라우저는 D 드라이브의 프로젝트 위치를 직접 추측하지 않습니다. 기본 URL과 href를 합쳐 목적지를 계산한 뒤 이동합니다. base 요소가 없다면 현재 문서 URL이 기준입니다.

수업 두 파일 공통 발췌:

```html
<a href="../asset/detail.html" target="_blank">디테일페이지 이동</a>
```

문서가 http://127.0.0.1:5500/03_상대주소/03_a.html이라고 가정합니다.

| href | 계산된 목적지의 경로 | 이유 |
| --- | --- | --- |
| 01_hello.html | /03_상대주소/01_hello.html | 현재 폴더에 붙임 |
| ../01_hello.html | /01_hello.html | 한 단계 위로 올라감 |
| ../asset/detail.html | /asset/detail.html | 부모 폴더의 asset으로 이동 |
| /asset/detail.html | /asset/detail.html | 현재 서버 origin의 루트부터 계산 |

같은 폴더가 아닌데 파일명만 쓰면 요청은 잘 만들어져도 존재하지 않는 URL로 가서 실패합니다. 두 수업 파일 모두 01_hello.html 링크가 이 문제를 가집니다. 현재 문서가 하위 폴더이므로 ../01_hello.html이 해당 구조에 맞습니다.

내 코드는 href="#bottom"과 id="bottom"을 함께 두었고 강사님 코드의 “맨 아래로” a에는 href가 없습니다. 내 코드는 대상 위치로 이동하고, 강사님 코드는 목적지가 없는 자리표시자입니다. #bottom은 fragment이며 HTTP 요청 대상 경로의 일부로 서버에 전송되는 값이 아닙니다.

내 이미지 링크에는 target="_blank" 뒤에 따옴표가 하나 더 있습니다. 오류 있는 원본은 비교 자료로 남기되 새로 쓰는 코드는 정상 따옴표로 고칩니다.

**디버깅:** 파일을 실제로 옮기기 전에 링크 주소를 복사해 계산 결과부터 확인합니다. Network의 404라면 URL과 서버 루트·파일 위치를 대조합니다. tel/mailto는 브라우저 설정과 연결 앱에 따라 처리되므로 모든 PC에서 전화·메일 화면이 동일하게 열린다고 단정하지 않습니다.
...
```javascript
new URL('../asset/detail.html', location.href).pathname
```

**예상 결과:** /asset/detail.html

서버 루트에서 해당 경로로 문서를 열었다는 전제입니다. 더 상위 URL 경로를 두고 제공하면 결과도 달라집니다. 실제 요청 성공 여부는 Network에서 별도로 확인합니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
...
## 브라우저 해석 복습 카드 — 현재 문서 URL에서 목적 URL 계산
