...
- [개념에서 실제 동작까지](#html-2)
- [1. 목록이 필요한 이유](#html-3)
- [2. 순서 없는 목록 `ul`](#html-4)
- [3. 목록 항목 `li`](#html-5)
- [4. 순서 있는 목록 `ol`](#html-6)
- [5. `ol`의 주요 속성](#html-7)
- [6. 중첩 목록](#html-8)
- [7. 설명 목록 `dl`, `dt`, `dd`](#html-9)
- [8. `ul`, `ol`, `dl` 비교](#html-10)
- [9. 목록과 링크](#html-11)
- [10. 목록의 기본 스타일과 CSS](#html-12)
- [11. 목록 마커 꾸미기](#html-13)
- [12. Emmet으로 목록 빠르게 작성하기](#html-14)
- [13. 접근성과 의미 구조](#html-15)
- [14. 내 코드와 강사님 코드 비교](#html-16)
- [15. 개선된 통합 예제](#html-17)
- [16. 자주 하는 실수](#html-18)
- [17. 실무 팁](#html-19)
- [18. 종합실습](#html-20)
- [19. 정답과 해설](#html-21)
- [20. 최종 체크리스트](#html-22)
- [21. 핵심 요약](#html-23)
- [브라우저 해석 복습 카드 — 목록 의미와 부모·자식 규칙](#html-24)
...
### 먼저 이해하기: 목록은 부모와 항목의 관계다

목록은 점이나 번호를 만드는 장식이 아니라 관련 항목을 묶는 구조입니다. ul은 순서를 바꿔도 핵심 의미가 유지되는 묶음, ol은 순서가 의미를 갖는 묶음입니다. 수업 메모의 “ol은 거의 사용하지 않는다”는 사용 금지 규칙이 아닙니다. 설치 절차와 순위에는 ol이 적합합니다.
...
| 처리 | 결과 |
| --- | --- |
| 부모 li 생성 | 리스트2라는 한 항목 구성 |
| 내부 ul 생성 | 그 항목에 속하는 하위 목록 구성 |
| 내부 li 생성 | 리스트2-1, 리스트2-2 표시 |

강사님 코드도 ul을 li 안에 두지만 그 부모 li에는 리스트2라는 설명 글자가 없습니다. 둘 다 중첩 구조를 사용하되 내 코드는 부모 항목의 이름을 보충했습니다.

내 파일에는 #list li의 display: inline-block 규칙이 있고 강사님 파일에는 없습니다. 숫자 1·2·3이 가로로 보이는 변화는 HTML 목록 종류 때문이 아니라 CSS 표시 방식 때문입니다. CSS가 마커를 숨기거나 배치를 바꾸더라도 작성한 구조를 확인해야 합니다.

💡 알아두면 좋은 점: dl은 용어와 설명 같은 이름-값 묶음에 쓰며, 항상 dt 하나에 dd 하나만 대응해야 하는 것은 아닙니다. 이 문서의 기본 예제부터 익힌 뒤 여러 용어·설명 묶음으로 확장합니다.

**직접 확인:** 설치→실행→확인 순서를 ol로 만듭니다. ul로 바꾸면 화면 번호와 순서 의미가 함께 달라지는지 비교하세요. ul>li*5는 편집기의 Emmet 약식 입력이지 브라우저가 해석하는 HTML 문법이 아닙니다.


### 개발자 도구에서 값을 읽고 확인하기

04_ul.html을 실제 브라우저에서 연 상태에서 개발자 도구의 Console에 다음을 입력합니다. 아래 코드는 HTML에 삽입할 태그가 아니라 브라우저의 JavaScript 확인 명령입니다. 💡 DOM·폼 API를 이용한 확인법으로, JavaScript 과정에서 더 자세히 배웁니다.
...
첫 ul에는 직접 자식 li가 네 개입니다. 후손 전체와 직접 자식을 구분합니다. 주석은 children에 포함되지 않습니다. HTML은 Python의 print가 없으므로 화면 결과와 Console에서 읽은 값을 분리해 확인합니다. 오류가 나면 명령을 입력한 페이지가 맞는지, 해당 요소가 실제로 있는지, 입력을 먼저 했는지 확인합니다.
