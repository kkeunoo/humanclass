...
- [13. ID 선택자 `#`](#css-16)
- [14. 클래스 선택자 `.`](#css-17)
- [15. 여러 조건을 동시에 만족하는 복합 선택자](#css-18)
- [16. 속성 선택자](#css-19)
- [17. 속성 존재 선택자 `[attr]`](#css-20)
- [18. 속성값 일치 선택자 `[attr=value]`](#css-21)
- [19. ID 선택자와 속성 선택자의 관계](#css-22)
- [20. 시작 문자열 선택자 `^=`](#css-23)
- [21. 끝 문자열 선택자 `$=`](#css-24)
- [22. 공백 구분 단어 선택자 `~=`](#css-25)
- [23. 추가 속성 선택자 확장](#css-26)
- [24. 결합자란?](#css-27)
- [25. 자손 결합자](#css-28)
- [26. 자식 결합자 `>`](#css-29)
- [27. 인접 형제 결합자 `+`](#css-30)
...
- [31. `:active`](#css-34)
- [32. `:focus`](#css-35)
- [33. `:focus-visible`](#css-36)
- [34. `:checked`](#css-37)
- [35. `:nth-child()`](#css-38)
- [36. `:nth-child()`의 흔한 오해](#css-39)
- [37. `:not()`](#css-40)
- [38. 가상 요소](#css-41)
- [39. `::before`](#css-42)
- [40. `::after`](#css-43)
- [41. 가상 요소와 DOM](#css-44)
- [42. `::selection`](#css-45)
- [43. 상속](#css-46)
- [44. 배경색이 자식에게 보이는 이유](#css-47)
- [45. 캐스케이드](#css-48)
- [46. 명시도](#css-49)
- [47. 원본 우선순위 실습 분석](#css-50)
- [48. 원본의 한 줄 우선순위표 보완](#css-51)
- [49. `!important`](#css-52)
- [50. 같은 명시도라면 뒤의 선언](#css-53)
- [51. 직접 지정한 값과 상속값](#css-54)
- [52. 테이블 선택자 실습](#css-55)
- [53. 사용자 정의 속성 주의](#css-56)
- [54. 선택자 작성 시 따옴표](#css-57)
- [55. 내 코드 분석](#css-58)
- [56. 내 코드 개선점](#css-59)
- [57. 강사님 코드 분석](#css-60)
- [58. 강사님 코드 개선점](#css-61)
- [59. 내 코드와 강사님 코드 비교](#css-62)
- [60. 불리언 속성 표기](#css-63)
- [61. 실무 선택자 설계 원칙](#css-64)
- [62. BEM 기초](#css-65)
- [63. 상태 클래스](#css-66)
- [64. 선택자 깊이 줄이기](#css-67)
- [65. 선택자 성능에 대한 현실적인 기준](#css-68)
- [66. CSS Reset과 기본 스타일](#css-69)
- [67. CSS 파일 구조 예](#css-70)
- [68. 권장 통합 예제](#css-71)
- [69. 개발자 도구로 선택자 확인하기](#css-72)
- [70. CSS가 적용되지 않을 때 점검 순서](#css-73)
- [71. 자주 하는 실수](#css-74)
- [72. 실무 개선 전후](#css-75)
- [종합실습](#css-76)
- [정답과 해설](#css-77)
- [최종 체크리스트](#css-78)
- [핵심 요약](#css-79)
- [렌더링 복습 카드 — 선택자 매칭과 캐스케이드](#css-80)
...
| 분류 | `02_CSS` |
| 원본 기준 | `workspace_html/css/01_selector.html`, `workspace_teacher/workspace_html/css/01_selector.html` |
| 핵심 범위 | CSS 문법, 적용 방법, 기본 선택자, 속성 선택자, 결합자, 가상 클래스, 가상 요소, 상속, 캐스케이드, 명시도 |
| 프로젝트 연결 | 공통 스타일, 메뉴, 폼 상태, 게시판, 컴포넌트 스타일, 유지보수 가능한 CSS |

> 이 문서는 내 코드와 강사님 코드의 `01_selector.html`을 비교해 CSS 적용 방법과 선택자 동작을 정리한다. 원본의 부정확한 설명은 수정하고, 클래스 중심 선택자 설계와 개발자 도구 검수 방법까지 실무 흐름으로 연결한다.
...
### CSS란? 요소의 표현을 결정하는 규칙

CSS는 DOM 요소에 색·크기·간격·배치를 지정하는 스타일 언어입니다. “뒤에 쓴 코드가 무조건 이긴다”가 아니라, 해당 요소와 속성에 적용되는 규칙들을 먼저 모아 캐스케이드로 값을 선택합니다. 선택자는 대상을 고르고 선언은 바꿀 속성과 값을 지정합니다.

수업 두 파일의 공통 발췌:

```css
#div2 { border: 1px solid aqua; }
[human] { border: 1px solid pink !important; }
.c4 { border: 1px solid blue; }
div { border: 1px solid red; }
```

대상은 div이며 id="div2", class="c4", human 속성과 검은색 인라인 border가 있습니다.

| 처리 단계 | 이 실습에서의 판단 |
| --- | --- |
| 선택자 매칭 | 위 네 규칙과 인라인 선언 모두 같은 요소에 해당 |
| 속성별 후보 모으기 | border의 폭·스타일·색 후보가 충돌 |
| 중요도 판별 | 작성자 important 선언이 일반 인라인 선언보다 우선 |
| 값 결정 | 1px solid pink 테두리 |
| 실제 표시 | 다른 크기·배경 속성과 합쳐 그려짐 |

두 파일의 이 실습은 같은 핵심 결과를 갖습니다. 인라인 검정이 있어도 pink가 되는 이유를 설명하는 것이 목표입니다. important를 잠시 끄면 일반 선언 중 인라인 검정이 우선합니다. 인라인도 끄면 이 조건의 ID 규칙 aqua가 우선합니다. 한 요소의 border가 결정됐다고 다른 속성까지 한 규칙 전체가 이긴 것은 아닙니다.

💡 알아두면 좋은 점: 이 실습은 같은 작성자 출처의 단순한 규칙입니다. 실제 캐스케이드는 출처·중요도·레이어·명시도 등 여러 조건을 먼저 비교합니다. active transition 등 별도 우선 조건도 있어 important를 모든 상황의 절대 최상위로 외우지 않습니다. [캐스케이드 표준](https://www.w3.org/TR/css-cascade-5/)을 참고합니다.

**확인:** Styles에서 취소선 선언과 최종 border를 함께 봅니다. 일단 important를 추가하기 전에 선택자 매칭·파일 로딩·충돌 속성을 확인하세요.


### 개발자 도구에서 실제 값을 읽기

01_selector.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
getComputedStyle(document.querySelector('#div2')).borderTopColor
```

**예상 결과:** rgb(255, 192, 203)

important 규칙이 살아 있는 원본 기준입니다. 선언을 끄면 결과가 달라집니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** ID 선택자와 [id=...]는 같은 요소를 고르면 명시도도 같은가?

**해설:** 아닙니다. 같은 대상을 고를 수 있지만 ID 선택자와 속성 선택자의 명시도는 다릅니다.
...
## 렌더링 복습 카드 — 선택자 매칭과 캐스케이드

브라우저는 DOM 요소와 선택자를 대조한 뒤 중요도, 출처, 레이어, 우선순위, 범위 근접성, 작성 순서 등을 거쳐 승리 선언을 정한다. 단순히 '아래에 쓴 것이 항상 이긴다'가 아니다.

`.card .title`이 `.title`보다 구체적이지만 인라인 스타일이나 `!important` 등 다른 조건도 함께 작용한다. Elements의 Styles에서 취소선과 규칙 출처를 확인하고 Computed에서 최종값을 본다.

**원본 연결:** 내 코드와 강사님 코드의 `workspace_html/css/01_selector.html`에서 실제 선택자·계산값·화면 차이를 확인한다.
