...
- [개념에서 실제 동작까지](#css-3)
- [1. CSS 박스 모델이란?](#css-4)
- [2. 박스 모델을 그림으로 이해하기](#css-5)
- [3. `width`와 `height`](#css-6)
- [4. `width`와 블록 요소](#css-7)
- [5. `height` 사용 시 주의](#css-8)
- [6. `margin`](#css-9)
- [7. `margin` 축약형](#css-10)
- [8. 원본의 연속된 `margin` 선언](#css-11)
- [9. 음수 마진](#css-12)
- [10. 논리 속성 `margin-inline`, `margin-block`](#css-13)
- [11. `padding`](#css-14)
- [12. `padding` 축약형](#css-15)
- [13. `padding`과 배경색](#css-16)
- [14. `margin`과 `padding` 비교](#css-17)
- [15. `border`](#css-18)
- [16. 방향별 테두리](#css-19)
- [17. 원본의 테두리 덮어쓰기](#css-20)
- [18. 테두리 스타일](#css-21)
- [19. `border-radius`](#css-22)
- [20. 원형 프로필 이미지](#css-23)
- [21. 원본 이미지 접근성 개선](#css-24)
- [22. 말풍선 모서리](#css-25)
- [23. 말풍선 개선 예](#css-26)
- [24. `%` 너비](#css-27)
- [25. `min-width`와 `max-width`](#css-28)
- [26. 작은 화면에서 `min-width` 주의](#css-29)
- [27. `margin: auto` 가운데 정렬](#css-30)
- [28. 원본 설명 보완](#css-31)
- [29. `width: fit-content`](#css-32)
- [30. `box-sizing`](#css-33)
- [31. `content-box`](#css-34)
- [32. `border-box`](#css-35)
- [33. `content-box`와 `border-box` 비교](#css-36)
- [34. 전역 `box-sizing`](#css-37)
- [35. Reset CSS](#css-38)
- [36. `body` 높이 `400vh`](#css-39)
- [37. 마진 상쇄란?](#css-40)
- [38. 형제 사이의 마진 상쇄](#css-41)
- [39. 부모와 첫 자식의 마진 상쇄](#css-42)
- [40. 주석과 공백은 상쇄를 막는가?](#css-43)
- [41. 마진 상쇄 계산 규칙](#css-44)
- [42. 마진 상쇄가 발생하지 않는 대표 상황](#css-45)
- [43. 원본의 해결 방법](#css-46)
- [44. `overflow: hidden` 해결의 장단점](#css-47)
- [45. `display: flow-root`](#css-48)
- [46. 패딩으로 내부 간격 표현](#css-49)
- [47. `gap` 사용](#css-50)
- [48. `orig` 실습](#css-51)
- [49. 내 코드 분석](#css-52)
- [50. 내 코드 개선점](#css-53)
...
- [55. 실무 컨테이너 패턴](#css-58)
- [56. 카드 패턴](#css-59)
- [57. 박스 그림자와 크기](#css-60)
- [58. 입력 요소와 `border-box`](#css-61)
- [59. `outline`과 `border`](#css-62)
- [60. 개발자 도구 박스 모델 패널](#css-63)
- [61. 크기가 예상보다 클 때 점검 순서](#css-64)
- [62. 간격이 예상과 다를 때 점검 순서](#css-65)
- [63. 자주 하는 실수](#css-66)
- [종합실습](#css-67)
- [정답과 해설](#css-68)
- [최종 체크리스트](#css-69)
- [핵심 요약](#css-70)
- [렌더링 복습 카드 — 콘텐츠에서 바깥 크기까지](#css-71)
...
### 박스 모델이란? 지정 너비와 바깥 너비를 구분하는 규칙

내용(content), 내부 간격(padding), 테두리(border), 바깥 간격(margin)을 구분하면 “width는 300인데 왜 더 크게 보이지?”를 계산할 수 있습니다. margin은 border 바깥이며 배경색이 칠해지는 내부 공간이 아닙니다.

두 CSS 파일의 공통 발췌:

```css
.box { border: 1px solid red; width: 300px; height: 300px; margin: 50px; }
.box-sizing { padding: 50px; box-sizing: border-box; }
```

| 원본 대상 | 콘텐츠 너비 | border까지 너비 | 좌우 margin 포함 점유 합 |
| --- | --- | --- | --- |
| .box만 있음 | 300px | 302px | 402px |
| .box.box-sizing | 198px | 300px | 400px |

기본 .box에는 padding이 없습니다. border-box 상자는 선언된 300 안에 padding 100과 border 2를 포함하므로 content는 198입니다. 위 합은 수평 방향 계산이며 세로 margin 상쇄나 겹침을 무시한 화면 좌표를 보장하는 표가 아닙니다.

실제 차이: 내 #auto 너비는 250px, 강사님은 150px입니다. 두 파일 모두 margin: 10px auto로 일반 블록의 남은 수평 공간을 양쪽에 나눕니다. “100%가 아니면 어디서나 auto 중앙 정렬”이 아니라 배치 문맥과 남는 공간을 함께 봅니다.

연속 margin 선언은 누적되지 않습니다. 같은 규칙 안에서 마지막 margin: 30px가 앞선 축약값을 덮습니다. 두 양수 세로 형제 margin 30과 50은 상쇄 조건에서 80이 아닌 50이 됩니다. 음수까지 들어가면 단순히 큰 값만 고르는 규칙이 아니므로 뒤의 계산 예제를 읽습니다.

**확인:** Box Model 패널에서 content, padding, border, margin을 순서대로 읽고 실제 측정값과 계산을 대조합니다. overflow: hidden은 일부 상쇄 문제를 피하면서 콘텐츠도 자를 수 있어 만능 해결책이 아닙니다. 💡 flow-root는 새 블록 서식 문맥을 만드는 별도 대안입니다.


### 개발자 도구에서 실제 값을 읽기

03_box.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
Array.from(document.querySelectorAll('.box'), e => [getComputedStyle(e).boxSizing, e.getBoundingClientRect().width])
```

**예상 결과:** [['content-box', 302], ['border-box', 300]]

원본 첫 .box에는 padding이 없습니다. CSS와 border가 그대로 로드된 상태입니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** 첫 원본 .box의 border 박스 너비가 402인가?

**해설:** 아닙니다. width 300과 좌우 border 1씩으로 302입니다. padding 50을 추가한 별도 실험이면 402입니다.
...
위 코드는 원본에 padding: 50px를 추가한 💡 비교 실험입니다. 실제 원본 첫 .box는 padding이 없어 border 박스가 300 + 1 + 1 = 302px입니다. 일반 content-box와 border-box를 같은 padding 조건으로 비교한 실험과 원본의 서로 다른 두 박스를 구분합니다.
...
```text
content-box 요소(원본 첫 .box, padding 없음):
테두리 박스 302 × 302px
