...
- [개념에서 실제 동작까지](#css-3)
- [1. 원본 실습 구조](#css-4)
- [2. 원본 결과 한눈에 비교](#css-5)
- [3. `visibility`](#css-6)
- [4. `visibility: hidden`의 공간 유지](#css-7)
- [5. `visibility`와 자식 요소](#css-8)
- [6. `display: none`](#css-9)
- [7. `display: none`과 문서 구조](#css-10)
- [8. HTML 주석과 `display: none`](#css-11)
- [9. `opacity`](#css-12)
- [10. `opacity: 0`의 핵심 동작](#css-13)
- [11. 내 코드의 설명 보완](#css-14)
- [12. 투명한 요소의 클릭 문제](#css-15)
- [13. 키보드 포커스 문제](#css-16)
- [14. `opacity`는 자식 전체에 적용된다](#css-17)
- [15. 배경만 투명하게 만들기](#css-18)
- [16. `opacity` 값의 유효 범위](#css-19)
- [17. 숨김 방식 핵심 비교](#css-20)
- [18. HTML `hidden` 속성](#css-21)
- [19. `aria-hidden`](#css-22)
- [20. visually hidden 패턴](#css-23)
- [21. `inert` 속성 확장 학습](#css-24)
- [22. 팝업 닫기 상태](#css-25)
- [23. 메뉴 열기와 닫기](#css-26)
- [24. 아코디언 패널](#css-27)
- [25. `display`는 단순 전환되지 않는다](#css-28)
- [26. 페이드 인·아웃 기본 패턴](#css-29)
- [27. `transition`과 `visibility`](#css-30)
- [28. 애니메이션 감소 설정](#css-31)
- [29. 로딩 상태에서의 `opacity`](#css-32)
- [30. 비활성 상태의 대비](#css-33)
- [31. 호버 효과에서 `opacity`](#css-34)
- [32. 이미지 오버레이](#css-35)
- [33. 내 코드 분석](#css-36)
- [34. 내 코드 개선점](#css-37)
- [35. 강사님 코드 분석](#css-38)
- [36. 강사님 코드 개선점](#css-39)
- [37. 내 코드와 강사님 코드 비교](#css-40)
- [38. 원본을 개선한 기본 비교 예제](#css-41)
- [39. 실무 상태 클래스](#css-42)
- [40. 상태 속성 중심 설계](#css-43)
- [41. 접근 가능한 모달 기본 구조](#css-44)
- [42. 페이드 모달 예제](#css-45)
- [43. 로딩 플레이스홀더](#css-46)
- [44. 요소가 보이지 않을 때 점검 순서](#css-47)
- [45. 빈 공간이 남을 때 점검 순서](#css-48)
- [46. 보이지 않는 요소가 클릭될 때 점검](#css-49)
- [47. 자주 하는 실수](#css-50)
- [종합실습](#css-51)
- [정답과 해설](#css-52)
- [최종 체크리스트](#css-53)
- [핵심 요약](#css-54)
- [렌더링 복습 카드 — 보임·공간·상호작용의 차이](#css-55)
...
### 숨김이란? 보임·자리·조작 가능성을 따로 바꾸는 일

화면에서 안 보인다는 사실만으로 요소가 없거나 클릭할 수 없다고 판단하면 안 됩니다. 수업은 같은 다섯 문단 중 세 문단의 숨김 방식을 비교합니다.
...
| 대상 문단 | 화면 | 배치 자리 | 일반적인 조작·접근성 |
| --- | --- | --- | --- |
| 두번째 .hidden | 안 보임 | 남음 | 해당 숨김 요소는 보통 조작·접근성 노출 제외 |
| 세번째 .none | 안 보임 | 제거 | 렌더링·일반 접근성 노출 제외 |
| 네번째 .opacity | 투명 | 남음 | 투명도만으로 클릭·포커스·접근성 노출을 막지 않음 |

문단 자체는 기본 키보드 포커스 대상이 아닙니다. opacity의 포커스 문제를 실험하려면 버튼·링크처럼 원래 포커스 가능한 요소를 넣어야 합니다. 따라서 원본 p에서 Tab이 안 갔다고 opacity가 포커스를 차단한다고 결론내리지 않습니다.

내 주석의 “visibility:hidden과 opacity:0은 동일”은 자리 유지라는 한 측면만 같습니다. 원본의 살아 있는 선언은 두 파일 모두 opacity: 0이며, 주석 속 0.3·0.7은 현재 결과를 바꾸지 않습니다.

💡 재현 예제:

```html
<button style="opacity: 0">투명한 버튼</button>
```

다른 차단 조건이 없다면 Tab으로 포커스를 받을 수 있습니다. 완전히 닫힌 패널을 구현할 때는 시각·입력·접근성 상태를 함께 관리합니다. pointer-events: none만으로 키보드 포커스까지 해결된다고 생각하지 않습니다.

**확인:** Elements에는 세 요소가 모두 남는지, .none의 표시 박스가 사라지는지, 두 빈 공간은 유지되는지 비교합니다. DOM 존재와 레이아웃 박스 존재를 구분하세요.


### 개발자 도구에서 실제 값을 읽기

05_opacity.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
[document.querySelector('.none').getClientRects().length, getComputedStyle(document.querySelector('.opacity')).opacity]
```

**예상 결과:** [0, '0']

none은 박스가 생성되지 않고 opacity는 투명도 값이 0입니다. DOM에는 두 요소가 남습니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** 투명 p에 Tab이 안 가면 opacity가 포커스를 막는가?

**해설:** 아닙니다. 일반 p는 기본 포커스 대상이 아니므로 원래 포커스 가능한 버튼으로 비교해야 합니다.
