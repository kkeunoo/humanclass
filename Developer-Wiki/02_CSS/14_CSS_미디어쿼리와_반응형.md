...
- [개념에서 실제 동작까지](#css-3)
- [1. 반응형 웹이란?](#css-4)
- [2. 미디어 쿼리 기본 문법](#css-5)
- [3. 원본 미디어 타입 `screen`](#css-6)
- [4. `max-width: 600px`](#css-7)
- [5. 원본의 주석 처리된 `min-width`](#css-8)
- [6. `max-width`와 `min-width`](#css-9)
- [7. 데스크톱 우선과 모바일 우선](#css-10)
- [8. 원본 기본 `ul`](#css-11)
- [9. 원본 기본 `li`](#css-12)
- [10. `inline-block` 메뉴](#css-13)
- [11. Inline-block 사이 공백](#css-14)
- [12. `line-height` 세로 중앙 정렬](#css-15)
- [13. 원본 기본 `span`](#css-16)
- [14. 모바일에서 메뉴 항목 변경](#css-17)
- [15. `width: 100%`와 Border](#css-18)
- [16. 모바일에서 Span 표시](#css-19)
- [17. 모바일 `ul` 숨김 방식](#css-20)
- [18. `opacity: 0`의 중요한 특징](#css-21)
- [19. `display: none`과 비교](#css-22)
- [20. `position: relative`와 `top`](#css-23)
- [21. 원본 인접 형제 선택자](#css-24)
- [22. 원본 Hover 상태](#css-25)
- [23. Transition 위치 문제](#css-26)
- [24. `transition: all` 개선](#css-27)
- [25. 원본 메뉴가 유지되지 않는 문제](#css-28)
- [26. Hover 영역 확장 방법](#css-29)
- [27. 모바일에서 Hover의 한계](#css-30)
- [28. `span`의 의미 문제](#css-31)
- [29. 접근 가능한 메뉴 버튼](#css-32)
- [30. 원본 `li`에 링크가 없음](#css-33)
- [31. 시맨틱 내비게이션](#css-34)
- [32. CSS만 사용하는 Focus-within 예제](#css-35)
- [33. JavaScript 토글 방식](#css-36)
- [34. `hidden`과 Transition](#css-37)
- [35. 메뉴 닫기 동작](#css-38)
- [36. 브레이크포인트 선택](#css-39)
- [37. 뷰포트 Meta 태그](#css-40)
- [38. 화면 너비와 요소 너비](#css-41)
- [39. Container Query 확장 학습](#css-42)
- [40. 화면 방향 조건](#css-43)
- [41. Hover 가능 여부 조건](#css-44)
- [42. Reduced Motion](#css-45)
- [43. Print 미디어 쿼리](#css-46)
- [44. 원본 문서 언어와 제목](#css-47)
- [45. 내 코드와 강사님 코드 동일성](#css-48)
- [46. 공통 코드의 장점](#css-49)
- [47. 공통 코드의 개선점](#css-50)
- [48. 내 코드와 강사님 코드 비교](#css-51)
- [49. 원본 보존 코드](#css-52)
- [50. CSS 중심 개선 예제](#css-53)
- [51. 실무형 버튼 토글 예제](#css-54)
- [52. 모바일 우선 개선 예제](#css-55)
- [53. Breakpoint에서 상태 초기화](#css-56)
- [54. 터치 영역](#css-57)
- [55. 메뉴 아이콘 접근성](#css-58)
- [56. 메뉴가 보이지 않을 때 점검](#css-59)
- [57. 메뉴가 클릭되지 않을 때 점검](#css-60)
- [58. 가로 Overflow 점검](#css-61)
- [59. 자주 하는 실수](#css-62)
- [종합실습](#css-63)
- [정답과 해설](#css-64)
- [최종 체크리스트](#css-65)
- [핵심 요약](#css-66)
- [렌더링 복습 카드 — 미디어 조건 평가와 규칙 전환](#css-67)
...
### 미디어 쿼리란? 환경 조건에 맞을 때만 규칙을 후보로 넣는 기능

미디어 쿼리는 브라우저를 다른 HTML로 바꾸는 기능이 아닙니다. 뷰포트 등 조건을 평가해 적용 가능한 CSS 규칙을 선택합니다. 조건 안의 규칙도 캐스케이드 충돌을 거치므로 조건이 맞았다고 무조건 모든 기존 값을 덮지 않습니다.

두 수업 원본은 주석·공백 차이를 제외한 핵심 규칙과 화면 구조가 같습니다.

```css
@media screen and (max-width: 600px) {
    li { display: block; width: 100%; }
    span { display: inline; }
    ul { opacity: 0; position: relative; top: -20px; }
}
```

| 뷰포트 너비 | 조건 | 기본 메뉴 결과 |
| --- | --- | --- |
| 601 CSS px | 거짓 | li inline-block, span 숨김 |
| 600 CSS px | 참 | li block·100%, span 표시, ul 투명 |
| 599 CSS px | 참 | 동일 모바일 규칙 후보 |

기기 물리 픽셀 수나 전체 모니터 크기가 아니라 쿼리가 보는 CSS 뷰포트 너비입니다. 개발자 도구의 창 너비와 쿼리 평가도 함께 확인합니다.

원본은 span:hover + ul 조건으로 메뉴를 보입니다. +는 바로 다음 형제여야 하고, ul로 마우스를 옮기면 span:hover가 끝날 수 있습니다. “한 번 누르면 열린 상태 유지”를 구현한 것이 아닙니다. span을 버튼처럼 보이게만 만든 것도 키보드 버튼 기능이 아닙니다.

opacity:0 상태는 자리와 잠재적 상호작용을 남깁니다. 모바일 터치에서 hover만 의존하면 조작이 불안정할 수 있습니다. 💡 버튼·aria-expanded·실제 토글 상태와 포커스 관리로 확장하는 예제는 수업 원본과 구분합니다.

**확인:** 599·600·601을 각각 시험하고, 표시·크기·클릭 가능성·키보드 접근을 별도로 기록합니다. width:100%에 border가 더해지는 content-box의 넘침도 계산합니다.


### 개발자 도구에서 실제 값을 읽기

14_media.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
matchMedia('(max-width: 600px)').matches
```

**예상 결과:** 601px에서 false / 600px·599px에서 true

기기 이름이 아니라 CSS 뷰포트 기준입니다. 모바일 에뮬레이션에서도 실제 평가 결과를 확인합니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** 조건이 참이면 원본 메뉴의 자리는 사라지는가?

**해설:** 아닙니다. opacity:0이므로 자리 제거가 아니라 투명한 상태입니다.
