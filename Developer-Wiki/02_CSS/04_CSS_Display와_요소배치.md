...
- [개념에서 실제 동작까지](#css-3)
- [1. `display`란?](#css-4)
- [2. HTML 요소의 기본 `display`](#css-5)
- [3. 원본의 첫 번째 블록 예제](#css-6)
- [4. 블록 요소의 너비](#css-7)
- [5. 블록 요소는 새 줄에서 시작한다](#css-8)
- [6. 블록 요소 가운데 정렬](#css-9)
- [7. `inline` 요소란?](#css-10)
- [8. 인라인 요소는 콘텐츠 흐름에 참여한다](#css-11)
- [9. 인라인 요소의 `width`와 `height`](#css-12)
- [10. 인라인 요소의 수평 마진](#css-13)
- [11. 인라인 요소의 수직 마진](#css-14)
- [12. 인라인 요소의 패딩](#css-15)
- [13. 인라인 요소의 배경색](#css-16)
- [14. `display: block`](#css-17)
- [15. 인라인 안에 블록 요소를 넣은 원본](#css-18)
- [16. 브라우저의 DOM 자동 보정](#css-19)
- [17. `display: inline`](#css-20)
- [18. `text-align`](#css-21)
- [19. 자식에 `text-align`을 지정한 원본](#css-22)
- [20. 블록 요소와 인라인 콘텐츠의 가운데 정렬 비교](#css-23)
- [21. `inline-block`](#css-24)
- [22. 원본의 `inline-block` 예제](#css-25)
- [23. `inline-block`과 `margin: auto`](#css-26)
- [24. `inline-block`의 기준선 정렬](#css-27)
- [25. `vertical-align`](#css-28)
- [26. `vertical-align: middle`의 의미](#css-29)
- [27. `inline-block` 사이의 공백](#css-30)
- [28. `font-size: 0` 방식](#css-31)
- [29. 음수 마진으로 공백 제거](#css-32)
- [30. 태그 사이 공백 제거 방식](#css-33)
- [31. Flexbox로 인라인 블록 공백 해결](#css-34)
- [32. 원본의 `-webkit-` 주석](#css-35)
- [33. `display: none`](#css-36)
- [34. `display: none`과 HTML 주석의 차이](#css-37)
- [35. JavaScript로 표시 상태 변경](#css-38)
- [36. HTML `hidden` 속성](#css-39)
- [37. `display: none`과 접근성](#css-40)
- [38. `visibility: hidden`](#css-41)
- [39. `opacity: 0`](#css-42)
- [40. 숨김 방식 비교](#css-43)
- [41. `display: table`](#css-44)
- [42. `display: table-cell`과 `vertical-align`](#css-45)
- [43. 실제 표와 CSS 테이블 레이아웃](#css-46)
- [44. Flexbox로 수직 가운데 정렬](#css-47)
- [45. `display`와 HTML 의미](#css-48)
- [46. `display`의 바깥쪽과 안쪽 개념](#css-49)
- [47. `display: contents` 확장 학습](#css-50)
- [48. 내 코드 분석](#css-51)
- [49. 내 코드 개선점](#css-52)
- [50. 강사님 코드 분석](#css-53)
- [51. 강사님 코드 개선점](#css-54)
- [52. 내 코드와 강사님 코드 비교](#css-55)
- [53. 원본 통합 개선 예제](#css-56)
- [54. Flexbox로 개선한 나란히 배치](#css-57)
- [55. Grid로 개선한 카드 목록](#css-58)
- [56. 버튼에 `inline-block`이 필요한가?](#css-59)
- [57. 이미지 아래 공백과 인라인 기준선](#css-60)
- [58. `display: none` 전환 애니메이션 주의](#css-61)
- [59. 개발자 도구로 `display` 확인하기](#css-62)
- [60. 요소가 옆으로 배치되지 않을 때 점검](#css-63)
- [61. 가운데 정렬이 안 될 때 점검](#css-64)
- [62. 수직 정렬이 안 될 때 점검](#css-65)
- [63. 자주 하는 실수](#css-66)
- [종합실습](#css-67)
- [정답과 해설](#css-68)
- [최종 체크리스트](#css-69)
- [핵심 요약](#css-70)
- [렌더링 복습 카드 — 박스 생성 방식과 정상 흐름](#css-71)
...
### display란? 박스가 주변 흐름과 내부 내용을 배치하는 방식

display는 HTML 의미를 바꾸는 속성이 아닙니다. 기본 div를 inline으로 표시해도 문법상 모든 인라인 요소 안에 div를 넣을 수 있게 되는 것은 아닙니다. 박스 생성과 HTML의 허용 구조를 따로 판단합니다.
...
span 같은 비대체 inline을 대상으로 한 실습입니다. 선언이 문법적으로 존재하는 것과 그 배치에 width·height가 적용되는 것은 다릅니다. img 같은 대체 요소까지 “인라인이면 크기 지정 불가”로 확대하지 않습니다.

| 설정 | 이 실습의 효과 |
| --- | --- |
| width/height | 일반 inline span의 박스 크기를 블록처럼 정하지 않음 |
| 수평 margin/padding | 좌우 간격과 박스 영역에 영향 |
| 수직 margin | 일반 inline의 위아래 배치 간격을 블록처럼 만들지 않음 |
| 수직 padding | 배경·테두리 영역은 늘 수 있지만 줄 높이를 같은 방식으로 밀지 않아 겹침 가능 |

inline-block은 옆으로 흐르면서 너비·높이를 지정할 수 있습니다. 태그 사이 공백도 인라인 텍스트이므로 빈틈이 생깁니다. 그 폭은 글꼴·크기에 따라 달라 “무조건 4px”이 아닙니다. 원본의 font-size: 0과 자식 16px 복원은 공백 텍스트 크기를 줄이는 실험입니다.

강사님 04_dispaly.html과 내 04_display.html은 파일명 철자가 다릅니다. 실제 이름을 따라 열어야 하고, 외부 파일은 둘 다 asset/css/04_display.css입니다. 파일명과 링크가 다르다는 것만으로 CSS 오류라고 추론하지 않습니다.

**확인:** span을 선택해 width가 선언되어 있는지, Computed와 화면 너비가 어떤지 봅니다. inline-block으로 바꾼 다음 차이를 비교합니다. 재현한 변경은 원본과 구분해 기록합니다.


### 개발자 도구에서 실제 값을 읽기

04_display.html / 강사님 04_dispaly.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
getComputedStyle(document.querySelector('.inline')).display
```

**예상 결과:** inline

원본 span을 대상으로 하고 다른 실습 변경을 적용하지 않은 상태입니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** inline에 width:200px가 보이면 화면 너비도 200인가?

**해설:** 일반 비대체 inline에서는 그렇지 않습니다. 선언 유효성과 배치 적용을 구분합니다.
