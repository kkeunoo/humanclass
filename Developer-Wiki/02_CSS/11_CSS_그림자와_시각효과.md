...
- [1. CSS 그림자란?](#css-4)
- [2. 원본 실습 구조](#css-5)
- [3. `text-shadow` 기본 문법](#css-6)
- [4. 수평 오프셋](#css-7)
- [5. 수직 오프셋](#css-8)
- [6. 원본 `.red`](#css-9)
- [7. 내 코드 `.red` 주석 보완](#css-10)
- [8. Blur radius](#css-11)
- [9. 원본 `.blur`](#css-12)
- [10. “blur 투명도 처리” 설명 보완](#css-13)
- [11. 흰색 글자의 대비 문제](#css-14)
- [12. `font-size: 2em`](#css-15)
- [13. 색상 생략](#css-16)
- [14. 다중 텍스트 그림자](#css-17)
- [15. 텍스트 외곽선처럼 만들기](#css-18)
- [16. `box-shadow` 기본 문법](#css-19)
- [17. 원본 `.box`](#css-20)
- [18. 원본 박스 그림자 해석](#css-21)
- [19. 박스 높이](#css-22)
- [20. Spread radius](#css-23)
- [21. `inset`](#css-24)
- [22. 다중 박스 그림자](#css-25)
- [23. 원본 `.box2`](#css-26)
- [24. `:hover`](#css-27)
- [25. `cursor: pointer`](#css-28)
- [26. 클릭 가능한 카드라면](#css-29)
- [27. Hover와 Focus](#css-30)
- [28. 그림자 전환](#css-31)
- [29. 움직임 감소 설정](#css-32)
- [30. 그림자는 레이아웃 공간을 만들지 않는다](#css-33)
- [31. Overflow와 그림자](#css-34)
- [32. Border와 Shadow 차이](#css-35)
- [33. Focus ring으로서의 box-shadow](#css-36)
- [34. 그림자 색상](#css-37)
- [35. 그림자 디자인 토큰](#css-38)
- [36. 그림자 단계](#css-39)
- [37. 다크 모드 그림자](#css-40)
- [38. Text-shadow 접근성](#css-41)
- [39. 상태 전달을 그림자에만 의존하지 않기](#css-42)
- [40. 성능 고려](#css-43)
- [41. 원본 문서 언어와 제목](#css-44)
- [42. 인라인 `<style>`](#css-45)
- [43. 내 코드 분석](#css-46)
- [44. 강사님 코드 분석](#css-47)
- [45. 내 코드와 강사님 코드 비교](#css-48)
- [46. 원본 통합 개선 예제](#css-49)
- [47. 카드 그림자 실무 패턴](#css-50)
- [48. 드롭다운 그림자](#css-51)
- [49. 입력창 focus 효과](#css-52)
- [50. 그림자가 보이지 않을 때 점검](#css-53)
- [51. Hover 효과가 작동하지 않을 때 점검](#css-54)
- [52. 자주 하는 실수](#css-55)
- [종합실습](#css-56)
- [정답과 해설](#css-57)
- [최종 체크리스트](#css-58)
- [핵심 요약](#css-59)
- [렌더링 복습 카드 — 박스·글자 외곽의 페인팅](#css-60)
...
### 그림자란? 크기를 늘리는 대신 바깥을 그리는 시각 효과

box-shadow는 박스 외곽, text-shadow는 글자 외곽에 그림자를 그립니다. blur는 흐림 반경이지 opacity처럼 요소 전체 투명도를 설정하는 값이 아닙니다. 큰 그림자가 보여도 주변 요소를 밀어내는 margin이 자동 생기지 않습니다.

두 원본의 공통 동작 발췌:

```css
.blur { text-shadow: 2px 2px 4px red; color: white; font-size: 2em; }
.box2:hover { box-shadow: 4px 4px 6px gray; cursor: pointer; }
```

두 번째 규칙은 내 파일의 발췌이고 강사님은 grey를 사용합니다. gray와 grey는 같은 색 이름입니다.

| 값 | 의미 | 화면에서 관찰 |
| --- | --- | --- |
| 첫 4px | 가로 오프셋 | 오른쪽으로 이동한 그림자 |
| 두 번째 4px | 세로 오프셋 | 아래로 이동 |
| 6px | 흐림 반경 | 경계가 퍼지는 정도 |
| gray | 그림자 색 | 회색 그림자 |

원본 .box2는 기본 150×200의 내용 박스와 1px border입니다. hover로 그림자를 추가해도 border 박스 크기는 원칙적으로 그대로입니다. 부모 overflow로 그림자가 잘릴 수 있으므로 “그림자가 없다”를 선언 오류 하나로 결론내리지 않습니다.

cursor:pointer는 포인터 모양만 바꿉니다. div를 실제 버튼으로 바꾸거나 click 처리·키보드 조작을 생성하지 않습니다. 시각적 힌트와 실제 상호작용의 구현을 구분합니다.

**재현:** 같은 박스에서 hover 전후 rect.width/height를 비교합니다. 그림자 대신 border 두께를 늘리면 content-box에서 바깥 크기가 늘 수 있어 다른 결과입니다. 흰 글자에 그림자가 있어도 배경에 따라 읽기 어려울 수 있으므로 대비를 직접 확인합니다.


### 개발자 도구에서 실제 값을 읽기

11_shadow.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
getComputedStyle(document.querySelector('.box2')).boxShadow
```

**예상 결과:** hover 전 none / hover 중 회색 그림자 값

포인터가 .box2 밖에 있는지 확인합니다. 그림자가 있다고 실제 클릭 처리도 있는 것은 아닙니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** blur 반경은 요소 opacity인가?

**해설:** 아닙니다. 그림자 경계의 흐림 정도입니다. 요소 전체 투명도와 구분합니다.
