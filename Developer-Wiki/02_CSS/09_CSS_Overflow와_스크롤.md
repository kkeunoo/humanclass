...
- [1. Overflow란?](#css-4)
- [2. 원본 공통 박스](#css-5)
- [3. 고정 크기와 overflow 발생](#css-6)
- [4. `overflow` 기본값](#css-7)
- [5. `overflow: hidden`](#css-8)
- [6. hidden은 콘텐츠를 삭제하지 않는다](#css-9)
- [7. `overflow: hidden`의 다른 효과](#css-10)
- [8. `overflow: clip` 확장 학습](#css-11)
- [9. `overflow: scroll`](#css-12)
- [10. 긴 콘텐츠가 있는 scroll 박스](#css-13)
- [11. 짧은 콘텐츠가 있는 scroll 박스](#css-14)
- [12. 스크롤바는 운영체제마다 다르다](#css-15)
- [13. 축별 overflow](#css-16)
- [14. `overflow-y: scroll`](#css-17)
- [15. “x는 없어질 수도 있다” 설명 보완](#css-18)
- [16. `overflow: auto`](#css-19)
- [17. `scroll`과 `auto` 비교](#css-20)
- [18. 세로 스크롤 패널 실무 패턴](#css-21)
- [19. `overflow` 단축 속성의 두 값](#css-22)
- [20. 원본 가로 스크롤 부모](#css-23)
- [21. 원본 가로 목록 자식](#css-24)
- [22. `white-space: nowrap`](#css-25)
- [23. inline-block 사이 공백](#css-26)
- [24. Flexbox 가로 스크롤 개선](#css-27)
- [25. 모바일 탭 목록 패턴](#css-28)
- [26. 원본 Emmet 문자열 노출](#css-29)
- [27. 반복 `<br>` 문제](#css-30)
- [28. 문서 언어와 제목](#css-31)
- [29. 내 코드의 설명 장점](#css-32)
- [30. 내 코드 설명 개선점](#css-33)
- [31. 강사님 코드의 장점](#css-34)
- [32. 강사님 코드 개선점](#css-35)
- [33. 내 코드와 강사님 코드 비교](#css-36)
- [34. 원본 통합 개선 예제](#css-37)
- [35. 코드 블록 실무 패턴](#css-38)
- [36. 테이블 래퍼 패턴](#css-39)
- [37. 모달 내부 스크롤](#css-40)
- [38. 채팅 기록 영역](#css-41)
- [39. `overscroll-behavior`](#css-42)
- [40. 스크롤바 스타일 주의](#css-43)
- [41. 포커스 outline 잘림](#css-44)
- [42. 이미지 크롭과 overflow](#css-45)
- [43. 말줄임표와 overflow 연결](#css-46)
- [44. 개발자 도구 확인 항목](#css-47)
- [45. overflow가 발생했는지 검사](#css-48)
- [46. 콘텐츠가 잘릴 때 점검](#css-49)
- [47. 스크롤이 생기지 않을 때 점검](#css-50)
- [48. 원치 않는 가로 스크롤 점검](#css-51)
- [49. 자주 하는 실수](#css-52)
- [종합실습](#css-53)
- [정답과 해설](#css-54)
- [최종 체크리스트](#css-55)
- [핵심 요약](#css-56)
- [렌더링 복습 카드 — 콘텐츠 크기와 박스 경계](#css-57)
...
### Overflow란? 내용이 표시 경계를 넘을 때의 처리

overflow는 글자를 삭제하거나 DOM에서 빼는 속성이 아닙니다. 콘텐츠가 박스 경계를 넘을 때 보임·잘림·스크롤 방식을 정합니다. 크기를 제한하지 않아 요소가 내용만큼 자란다면 기대한 내부 스크롤이 안 생길 수 있습니다.
...
| height | 130px | 130px |
| 긴 글 | 있음 | 다른 Lorem 문자열 |
| hidden/scroll/auto 비교 | 있음 | 있음 |

글자가 달라 내용 높이도 달라질 수 있습니다. 더 좁은 표본이 보통 더 많이 줄바꿈되지만 정확한 줄 수는 글꼴·문자열·화면 조건에 따라 확인합니다.
...
hidden은 경계를 넘어 보이는 내용을 자르지만 프로그램적으로 스크롤 가능한 스크롤 컨테이너입니다. scroll은 스크롤 UI를 제공하는 방식이고 auto는 넘칠 때 필요에 따라 제공합니다. 운영체제의 오버레이 스크롤바 설정 때문에 막대의 상시 가시성만으로 값을 판별하지 않습니다.

**측정 순서:** 박스의 clientHeight 읽기→전체 내용의 scrollHeight 읽기→둘 비교→Computed overflow 확인→휠 또는 스크롤 조작. scrollHeight > clientHeight이면 세로 넘침을 의심할 수 있지만 border를 포함한 rect.height와 무조건 동일 기준은 아닙니다.

한 축만 scroll을 지정해도 다른 축의 visible이 계산 과정에서 auto가 되는 조합이 있습니다. “overflow-y면 가로는 절대 스크롤 불가”가 아닙니다. 💡 clip과 hidden도 스크롤 컨테이너 여부가 달라 별도로 비교합니다. [Overflow 표준](https://www.w3.org/TR/css-overflow-3/)을 참고합니다.

**확인:** 원본의 가로 목록은 nowrap과 inline-block 때문에 한 줄에 유지됩니다. 가로 합이 부모 300을 넘는지 간격·border까지 세어보고, overflow를 없애는 것과 콘텐츠를 줄이는 것을 구분합니다.


### 개발자 도구에서 실제 값을 읽기

09_overflow.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
getComputedStyle(document.querySelector('.box.auto')).width
```

**예상 결과:** 내 코드 130px / 강사님 코드 100px

같은 문서 제목이더라도 원본 폭이 다릅니다. 글꼴과 문장도 달라 줄 수는 실측해야 합니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** hidden이면 내용이 삭제되었는가?

**해설:** 아닙니다. 표시 경계가 잘리는 것이며 DOM의 내용과 구분합니다.
