...
- [2. 원본 실습 구조](#css-5)
- [3. 기본 LOVE 박스](#css-6)
- [4. `position: static`](#css-7)
- [5. 오프셋 속성](#css-8)
- [6. `position: relative`](#css-9)
- [7. `relative`의 원래 공간](#css-10)
- [8. `relative`는 레이아웃 재배치가 아니다](#css-11)
- [9. 퍼센트 오프셋 보완](#css-12)
- [10. `position: absolute`](#css-13)
- [11. 원본의 중복 `top`](#css-14)
- [12. 절대 위치의 기준 박스](#css-15)
- [13. “부모가 없으면 body 기준” 설명 보완](#css-16)
- [14. 위치 기준 부모 만들기](#css-17)
- [15. `absolute`의 blockification](#css-18)
- [16. 절대 위치 남용 주석 보완](#css-19)
- [17. 오른쪽 아래 절대 배치](#css-20)
- [18. body 크기 확장](#css-21)
- [19. `position: fixed`](#css-22)
- [20. `fixed`의 일반 흐름](#css-23)
- [21. `fixed`도 inline-block인가?](#css-24)
- [22. 고정 맨 위로 링크 접근성](#css-25)
- [23. 모바일 안전 영역](#css-26)
- [24. `position: sticky`](#css-27)
- [25. sticky의 동작 조건](#css-28)
- [26. sticky는 fixed와 완전히 같지 않다](#css-29)
- [27. sticky 메뉴 개선](#css-30)
- [28. `z-index`](#css-31)
- [29. 원본 겹침 위치](#css-32)
- [30. 원본 `z-index` 주석 보완](#css-33)
- [31. stacking context란?](#css-34)
- [32. 큰 숫자가 항상 이기지 않는 이유](#css-35)
- [33. hover 시 레이어 변경](#css-36)
- [34. 메뉴와 서브메뉴 구조](#css-37)
- [35. 내 코드와 강사님의 서브메뉴 차이](#css-38)
- [36. hover 서브메뉴의 문제점](#css-39)
- [37. CSS hover 영역 개선](#css-40)
- [38. 접근 가능한 메뉴 토글](#css-41)
- [39. 부모 안의 자식 박스](#css-42)
- [40. 원본 중앙 계산](#css-43)
- [41. 강사님 세미콜론 누락](#css-44)
- [42. `calc()` 공백](#css-45)
- [43. 고정값 계산의 유지보수 문제](#css-46)
- [44. `transform` 중앙 정렬](#css-47)
- [45. Flexbox 중앙 정렬](#css-48)
- [46. CSS 사용자 지정 속성](#css-49)
- [47. 변수 이름과 범위](#css-50)
- [48. 변수 fallback](#css-51)
- [49. 내 코드 분석](#css-52)
- [50. 내 코드 개선점](#css-53)
- [51. 강사님 코드 분석](#css-54)
- [52. 강사님 코드 개선점](#css-55)
- [53. 내 코드와 강사님 코드 비교](#css-56)
- [54. 원본 통합 개선 예제](#css-57)
- [55. 카드 배지 패턴](#css-58)
- [56. 이미지 오버레이 패턴](#css-59)
- [57. 모달 기본 위치](#css-60)
- [58. sticky 표 헤더](#css-61)
- [59. 개발자 도구 확인 항목](#css-62)
- [60. absolute가 엉뚱한 곳에 갈 때 점검](#css-63)
- [61. sticky가 작동하지 않을 때 점검](#css-64)
- [62. z-index가 작동하지 않을 때 점검](#css-65)
- [63. 자주 하는 실수](#css-66)
- [종합실습](#css-67)
- [정답과 해설](#css-68)
- [최종 체크리스트](#css-69)
- [핵심 요약](#css-70)
- [렌더링 복습 카드 — containing block과 좌표 계산](#css-71)
...
### position이란? 흐름 참여와 위치 기준을 정하는 속성

relative는 원래 흐름 자리를 유지한 채 시각적 위치를 옮깁니다. absolute는 일반 흐름에서 빠져 기준 박스에 대해 배치됩니다. 두 개념을 “새 레이어”라는 표현 하나로 뭉뚱그리면 위치 계산을 설명하기 어렵습니다.

원본 두 CSS의 공통 발췌:

```css
.parent .relative { position: relative; top: -20px; left: 10px; }
.parent .absolute { position: absolute; top: 0; left: 0; top: 30px; }
```

relative 대상은 원래 위치에서 위로 20·오른쪽으로 10 이동하고 원래 점유는 유지됩니다. absolute는 top이 두 번 있으므로 현재 값은 30px입니다. top 0과 30이 합쳐 30 이동하는 명령 순서가 아니라 같은 속성의 뒤 선언이 남은 것입니다.

기준 조상을 먼저 찾습니다. 일반적인 positioned 조상이 있으면 그 기준 박스를 사용하고, 없으면 initial containing block을 사용합니다. 이를 항상 body 박스라고 외우지 않습니다. 💡 transform 등도 기준 박스 생성에 영향을 줄 수 있습니다. absolute가 자식을 자동 absolute로 만드는 것도 아닙니다.

실제 차이: submenu의 top은 내 코드 1px, 강사님 -1px입니다. 바로 다음 형제라는 + 조건은 같고 표시 후 이동 방향이 반대입니다. 메뉴를 떠나 submenu로 이동하면 hover 조건이 끊길 수 있어 표시만 확인하지 말고 사용 흐름을 검사합니다.

공통 sticky에는 top: 0 뒤 top: 50px이 있습니다. 스크롤 조건에서 50px 기준이 살아 있습니다. sticky는 흐름 자리를 유지하고 자신의 컨테이너 범위에 제한되므로 무조건 화면에 끝까지 고정되는 fixed와 다릅니다.

**확인:** 기준 조상, top 계산값, 원래 자리, 스크롤 컨테이너를 순서대로 기록합니다. z-index 큰 숫자가 안 먹으면 다른 stacking context와 비교하고 숫자를 무작정 올리지 않습니다.


### 개발자 도구에서 실제 값을 읽기

08_position.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.
...
top:0 다음의 top:50px가 남습니다. top 값 확인이 sticky 동작 조건 전체를 검증하지는 않습니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** absolute 자식의 모든 자식도 absolute가 되는가?

**해설:** 아닙니다. position은 그런 식으로 자동 상속되지 않습니다.
...
여기서 150px은 **원본에 적힌 식의 결과**입니다. 원본 자식은 content 100px에 양쪽 border 1px씩이 있어 실제 border 박스는 102px입니다. 기준 내부 영역이 400px이면 정확한 시작점은 (400 - 102) / 2 = 149px입니다. 원본 식은 테두리 크기를 생략해 1px 차이가 생길 수 있습니다. 자식에 border-box를 적용해 전체 너비를 100으로 맞추거나, 뒤에서 설명하는 translate 중앙 정렬처럼 실제 자기 크기를 반영하는 개선을 비교합니다. 원본 코드 자체의 계산과 정확한 중앙 배치를 구분하세요.
...
이 원본처럼 길이를 숫자 2로 나누는 계산은 calc의 기본 산술입니다. 임의의 단위끼리 나누는 다른 확장 문법과 구분합니다. +와 -의 양쪽 공백은 필수이며, 원본의 길이 / 숫자 계산을 막연히 지원 미확정 기능으로 외우지 않습니다.
...
이 식은 150px을 계산합니다. 고정된 크기 가정이 있으므로 자식 크기가 바뀌면 값을 다시 계산하거나 크기에 독립적인 중앙 정렬 방식을 사용합니다.
...
## 렌더링 복습 카드 — containing block과 좌표 계산
