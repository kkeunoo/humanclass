...
- [1. CSS 배경이란?](#css-4)
- [2. 원본 실습 구조](#css-5)
- [3. `background-color`](#css-6)
- [4. `background-image`](#css-7)
- [5. 외부 URL 사용 시 주의](#css-8)
- [6. 배경 이미지가 보이려면 크기가 필요하다](#css-9)
- [7. `background-repeat`](#css-10)
- [8. 원본의 `background-repeat`](#css-11)
- [9. 반복 패턴에 적합한 이미지](#css-12)
- [10. `background-size`](#css-13)
- [11. 고정 크기 `background-size`](#css-14)
- [12. 퍼센트 `background-size`](#css-15)
- [13. `background-size: cover`](#css-16)
- [14. `background-size: contain`](#css-17)
- [15. `cover`와 `contain` 비교](#css-18)
- [16. `background-position`](#css-19)
- [17. `background-position` 두 값의 순서](#css-20)
- [18. 프로필 배경 예제 분석](#css-21)
- [19. 프로필 이미지에는 `<img>`가 더 적절할 수 있다](#css-22)
- [20. `background-attachment`](#css-23)
- [21. `background-attachment: fixed`](#css-24)
- [22. 원본 `#back2`의 내부 스크롤](#css-25)
- [23. `background-attachment: local`](#css-26)
- [24. 모바일에서 `fixed` 대안](#css-27)
- [25. `background` 단축 속성](#css-28)
- [26. 단축 속성의 일반적인 작성 형태](#css-29)
- [27. 단축 속성이 기존 개별값을 초기화한다](#css-30)
- [28. 내 코드와 강사님 코드의 `#back3`](#css-31)
- [29. 다중 배경 이미지](#css-32)
- [30. 그라디언트도 배경 이미지다](#css-33)
- [31. 배경 오버레이](#css-34)
- [32. 배경 위 텍스트 접근성](#css-35)
- [33. `background-origin`](#css-36)
- [34. `background-clip`](#css-37)
- [35. 원본 문서 언어와 제목](#css-38)
- [36. 반복 `<br>` 문제](#css-39)
- [37. 내 코드 분석](#css-40)
- [38. 내 코드 개선점](#css-41)
- [39. 강사님 코드 분석](#css-42)
- [40. 강사님 코드 개선점](#css-43)
- [41. 내 코드와 강사님 코드 비교](#css-44)
- [42. 원본 개선 통합 예제](#css-45)
- [43. 실무 Hero 패턴](#css-46)
- [44. 카드 배경 패턴](#css-47)
- [45. 패턴 배경](#css-48)
- [46. 배경 이미지 성능](#css-49)
- [47. 반응형 배경 이미지](#css-50)
- [48. 배경 이미지가 안 보일 때 점검 순서](#css-51)
- [49. 이미지가 잘릴 때 점검 순서](#css-52)
- [50. 배경이 반복될 때 점검 순서](#css-53)
- [51. 자주 하는 실수](#css-54)
- [종합실습](#css-55)
- [정답과 해설](#css-56)
- [최종 체크리스트](#css-57)
- [핵심 요약](#css-58)
- [렌더링 복습 카드 — 이미지 로드와 박스 배경 그리기](#css-59)
...
### 배경 이미지란? 요소 박스에 그리는 이미지

배경은 별도의 콘텐츠 img가 아니라 요소의 배경 그림입니다. 의미 있는 사진을 설명해야 할 때는 alt가 가능한 img를 검토하고, 장식·질감·색 겹침은 배경으로 처리합니다. 배경은 보통 콘텐츠처럼 스스로 요소 높이를 만들지 않습니다.

수업 외부 CSS의 살아 있는 선언 발췌:
...
background-image URL은 원본의 외부 이미지입니다. 같은 파일의 앞선 주석 cover는 적용되지 않고 contain이 현재 동작합니다. URL 계산→자원 로드→배경 영역 결정→크기·위치·반복→표시 순서로 이해합니다. 외부 CSS의 상대 URL은 HTML 폴더가 아니라 CSS 파일 URL을 기준으로 합니다.

💡 이미지 400×200을 영역 200×200에 놓는 계산 예제:

| 방식 | 배율 | 표시 크기 | 결과 |
| --- | --- | --- | --- |
| contain | min(200/400, 200/200)=0.5 | 200×100 | 전부 보이지만 빈 영역 존재 |
| cover | max(200/400, 200/200)=1 | 400×200 | 영역을 덮고 일부가 잘림 |

잘리는 위치는 background-position이 결정합니다. contain이라고 자동 가운데 정렬까지 설정되는 것은 아닙니다. 원본 #back1과 profile의 위치 설정을 따로 봅니다.

두 파일의 #back3는 background 축약 후 background-size: contain을 따로 둡니다. 순서를 뒤집어 background를 나중에 쓰면 size 등 생략된 하위 속성이 초기화될 수 있습니다. 축약은 지정한 값만 추가하는 것이 아닙니다.

**확인:** Computed의 image·size·position과 Network 요청을 봅니다. 배경이 없으면 URL·응답 외에 높이가 있는지 먼저 확인합니다. fixed 배경의 모바일 동작을 데스크톱 결과 하나로 보장하지 않습니다.


### 개발자 도구에서 실제 값을 읽기

06_background.html을 브라우저에서 열고 Console에 다음을 입력합니다. 💡 JavaScript를 이용한 CSS 진단 명령이며 CSS 파일에 쓰는 코드가 아닙니다.

```javascript
getComputedStyle(document.querySelector('#back1')).backgroundSize
```

**예상 결과:** contain

외부 CSS가 로드되어 있고 cover 주석은 그대로인 기준입니다. 요소가 없다는 오류가 나오면 페이지와 선택 대상을 먼저 확인합니다. getComputedStyle은 API에서 계산·해석된 스타일을 읽고 getBoundingClientRect는 변환이 반영된 표시 경계 상자를 읽습니다. margin까지 포함한 전체 점유를 자동 반환하는 값은 아닙니다. CSS에는 Python의 print가 없으므로 화면 결과와 Console 값 확인을 구분합니다.

### 짧은 점검 문제와 해설

**문제:** background-size:cover를 주면 이미지가 왜곡되는가?

**해설:** 비율은 보존하며 영역을 덮도록 확대·축소하므로 일부가 잘릴 수 있습니다.
