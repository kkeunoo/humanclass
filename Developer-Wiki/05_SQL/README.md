...
> **상세 백과 개정 목표:** 이미 SQL을 아는 사람을 위한 요약이 아니라, 시간이 지나 내용을 잊어도 입력 Data부터 MariaDB 처리 과정과 Result Grid까지 다시 재현할 수 있는 개인 강의 백과사전입니다.

---

## 이번 개정 범위와 적용

01~20의 기존 상세 본문·종합실습·정답을 유지하며 개념 정의·입력 출처·실행 단계·Result Grid를 보강했다. 내·강사님 Script.sql 및 실제 초기화 SQL을 대조했다. 19·20의 개선·응용 코드를 별도 완료 수업 원본인 것처럼 만들지 않는다. 💡는 확장·반례·추가 검증 학습이다.

- [먼저 읽기 — 입력 데이터·SQL 실행·결과 재현](00_SQL_상세백과_읽기법.md)
- [비판적 검토와 수정·검증 기록](00_SQL_리팩토링_검토기록.md)

새 보충 예제와 반례 30개는 MariaDB 12.3.2의 분리된 검증 환경에서 결과를 확인했고, 오류 사례 5개와 EXPLAIN 전·후도 별도 확인했다. 기존 모든 SQL을 실행했다는 뜻은 아니다. DDL·DML은 wiki_demo_* 등의 새 실습 테이블로 구분하고 업무 DB에 그대로 복사하지 않는다.

이번 압축은 Developer-Wiki/05_SQL에 적용한다. 다른 파트·최상위 README·전체 용어색인은 이번에 변경하지 않았으며 전체 완료 후 통합 검수 대상이다.
