...
수업 원본 발췌, 설명용 재구성, 💡 확장 학습을 구분한다. 각 문서의 '개념에서 실제 실행까지'는 정의·입력 출처·처리 순서·원본 비교·출력·역추적·복습 해설을 연결한다. 기존 상세 번호형 본문과 예제는 축약하지 않았다. 비밀 키는 재노출하지 않는다.

메인 스레드 script는 파싱 위치에서 실행될 수 있으므로 위 전체 흐름은 단순화된 지도다. 외부 script의 defer는 파싱 후 실행하며 async는 다운로드 완료 시 실행되어 순서 보장과 다르다. DOMContentLoaded는 문서 파싱 완료, load는 추가 리소스 로드 완료를 다룬다.

[파트 목차](README.md) · [검토 기록](REFACTOR_REVIEW.md)
