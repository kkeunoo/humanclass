...
- [ ] 함수 호출이 현재 끝나야 다음 줄이 실행되는 동기 흐름을 설명할 수 있다.
- [ ] 타이머 등록과 타이머 콜백 실행 시점을 구분할 수 있다.
- [ ] 0ms가 정확한 실행 시각 보장은 아니라는 점을 설명할 수 있다.
- [ ] 현재 실행·microtask·다음 task의 출력 순서를 예측할 수 있다.
- [ ] await가 브라우저 전체를 멈추는 것이 아니라 async 함수 진행을 나누는 것을 설명할 수 있다.
- [ ] 콜백 안 오류와 HTTP 오류를 각각 어느 단계에서 처리할지 말할 수 있다.

## 이 문서에서 바로 찾기

- [개념에서 실제 실행까지: 비동기는 완료를 기다리지 않고 다음 코드로 진행하는 구조다](#js-14-section-2)
- [31. 동기와 비동기 비교](#js-14-section-33)
- [40. 자주 하는 실수](#js-14-section-42)

<details>
<summary>세부 목차 펼치기 — 번호형 본문 전체</summary>

- [학습 목표](#js-14-section-1)
- [개념에서 실제 실행까지: 비동기는 완료를 기다리지 않고 다음 코드로 진행하는 구조다](#js-14-section-2)
- [1. 동기 실행이란?](#js-14-section-3)
- [2. 함수 호출도 동기적으로 실행](#js-14-section-4)
- [3. Fn2 호출](#js-14-section-5)
- [4. Fn3 두 번 호출](#js-14-section-6)
- [5. 전체 동기 호출 흐름](#js-14-section-7)
- [6. 호출 스택](#js-14-section-8)
- [7. SetTimeout 0의 의미](#js-14-section-9)
- [8. 현재 동기 코드가 먼저 끝나는 이유](#js-14-section-10)
- [9. 원본 전체 출력 순서](#js-14-section-11)
- [10. 같은 0ms Timeout 순서](#js-14-section-12)
- [11. 내 코드의 표현 검토](#js-14-section-13)
- [12. “동기화가 안 되어 있다” 표현](#js-14-section-14)
- [13. 지연 함수라는 표현](#js-14-section-15)
- [14. Event Loop 개념](#js-14-section-16)
- [15. Web API](#js-14-section-17)
- [16. Task Queue](#js-14-section-18)
- [17. Event Loop 역할](#js-14-section-19)
- [18. 0ms가 즉시가 아닌 예제](#js-14-section-20)
- [19. Blocking](#js-14-section-21)
- [20. Single Thread 기본 이해](#js-14-section-22)
- [21. 내 코드의 주석 처리 예제](#js-14-section-23)
- [22. 강사님 코드의 주석 처리된 Console](#js-14-section-24)
- [23. 함수 선언 순서](#js-14-section-25)
- [24. Fn2 안의 Fn3 호출](#js-14-section-26)
- [25. Console 출력과 실제 실행 시간](#js-14-section-27)
- [26. Timeout ID](#js-14-section-28)
- [27. 취소 예제](#js-14-section-29)
- [28. Timeout Delay 생략](#js-14-section-30)
- [29. 중첩 Timeout](#js-14-section-31)
- [30. Timer 최소 지연](#js-14-section-32)
- [31. 동기와 비동기 비교](#js-14-section-33)
- [32. 원본 실행 단계 추적](#js-14-section-34)
- [33. 예상 출력 직접 작성 습관](#js-14-section-35)
- [34. My Code 분석](#js-14-section-36)
- [35. Teacher Code 분석](#js-14-section-37)
- [36. My Code vs Teacher Code](#js-14-section-38)
- [37. 대표 예제](#js-14-section-39)
- [38. 실무 활용: Loading 상태](#js-14-section-40)
- [39. 실행 순서 디버깅](#js-14-section-41)
- [40. 자주 하는 실수](#js-14-section-42)
- [41. 면접·복습 포인트](#js-14-section-43)
- [Problems](#js-14-section-44)
- [Answers & Explanations](#js-14-section-45)
- [Final Checklist](#js-14-section-46)
- [Key Summary](#js-14-section-47)
- [V3 실행 추적 카드 — 동기 스택 → Web API → 작업 큐 → 콜백](#js-14-section-48)

</details>

---

<a id="js-14-section-1"></a>

## 학습 목표

- 동기 코드·Promise·타이머의 출력 순서를 설명한다.
- 원본의 해당 부분을 찾아 실행 순서와 결과를 다시 확인한다.

---

<a id="js-14-section-2"></a>

## 개념에서 실제 실행까지: 비동기는 완료를 기다리지 않고 다음 코드로 진행하는 구조다

함수 안에서 다른 함수를 호출해 순서대로 끝내는 것은 동기 실행이다. setTimeout은 브라우저에 콜백을 예약하고 바로 반환하므로 다음 로그가 먼저 실행된다. 0ms는 즉시 중간에 끼어 실행한다는 뜻이 아니라 현재 실행이 끝난 뒤 실행 가능해지는 지연 조건이다.

두 수업 원본은 fn1→fn2→fn3 두 번→1→마지막을 먼저 출력하고, 이후 예약된2→3을 출력한다. 두 타이머가 같은 0ms라 순서대로 출력된다고 해서 서로 '동기화된 함수'가 되는 것은 아니다. 여전히 각 콜백이 나중 task로 실행된다.

💡 아래에는 수업 코드를 이해하기 위한 Promise를 한 줄 추가했다. 현재 script가 끝난 후 microtask가 처리되고 다음 타이머 task로 넘어가는 흐름을 본다. 긴 동기 계산이 있으면 타이머와 사용자 클릭 처리, 화면 갱신도 늦어질 수 있다. await는 해당 async 함수의 진행을 멈추는 것이지 브라우저 전체 스레드를 막는 명령이 아니다.

### 수업 원본에서 사용한 부분

아래는 전체 실행 파일이 아니라 **개념에 대응하는 문맥 발췌**다. 나머지 HTML·선언·등록 코드는 원본과 기존 번호형 본문에서 이어 확인한다. 

내 코드: `workspace_html/javascript/14_async.html`

```javascript
console.log('1')
                // 3이 먼저 나오는 이유는 setTimeout은 '0초 이후' 라는 개념이기 때문에,
                // 1-3-2 순서로 출력되게 됨 (이 모습이 동기화가 안 되어있는 것)
                // setTimeout( function() {
                //     console.log('2')
                // }, 0)
                // console.log('3')
...
강사님 코드: `workspace_teacher/workspace_html/javascript/14_async.html`
...
아무 확장 코드 없는 새 페이지 또는 Node에서 실행한다. 타이머의 실제 지연값은 보장하지 않고 순서만 확인한다. 이벤트 루프가 있다고 모든 연산이 병렬로 실행되는 것은 아니다.
...
**기본 실습:** 동기A→타이머T예약→PromiseP예약→동기B의 출력 순서를 쓰고 이유를 설명한다.

**응용·디버깅 실습:** 0ms타이머 앞에 긴 동기 반복이 있으면 실제 타이머는 언제 실행되는가?
...
1. A,B,P,T다. 현재 실행 완료→microtask→타이머task 흐름이다.
2. 긴 현재 실행이 끝난 뒤 실행할 수 있다. 지정지연시간이 정확한 실행시각 보장은 아니다.
...
**질문:** then의 콜백에서 return을 빼면 다음 then은 어떤 값을 받나?

**해설:** 그 콜백의 기본 반환값 undefined를 받는다. response.json()의 Promise를 return해야 다음 then이 파싱된 데이터를 기다려 받는다.

**한 번 더 확인:** 예제의 입력을 하나 바꾸고 결과를 먼저 예상한 뒤 실행한다. 정상 입력만 아니라 비어 있는 값, 경계값, 두 번 실행했을 때의 상태를 기존 실습·오류 절에서 반복 확인한다.

---

<a id="js-14-section-3"></a>
