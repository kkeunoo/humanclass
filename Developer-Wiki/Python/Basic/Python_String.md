...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 문자열은 입력과 출력에서 가장 자주 만나는 자료형이다. 인덱싱, 슬라이싱, 메서드를 알아야 필요한 부분을 추출하고 가공할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 슬라이싱의 시작은 포함되고 끝은 포함되지 않는다는 규칙을 작은 예제로 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 문자열은 변경할 수 없는 자료형이므로 메서드 결과는 새 문자열로 받아야 한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 마지막 인덱스와 length를 혼동하거나 슬라이싱 끝값이 포함된다고 생각하기 쉽다.
...
- 다음 문서 `Python_Tuple`로 넘어가기 전에 Check Point를 확인한다.
