...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `Hello와 실행`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
Hello와 실행은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 첫 실행은 개발 환경과 파일 실행 흐름이 올바른지 확인하는 단계다. 출력문과 들여쓰기의 기본을 익혀 이후 모든 예제의 출발점을 만든다.

> [!TIP]
> **⭐ 실무 TIP**  
> 짧은 코드를 직접 실행하고 오류 메시지에 표시된 파일명과 줄 번호를 먼저 읽는다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> Python은 들여쓰기를 코드 블록 구분에 사용한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 다른 언어처럼 중괄호로 블록을 만든다고 생각하거나 들여쓰기를 섞으면 오류가 발생한다.
...
Python 파일은 `.py` 확장자를 사용한다. 위에서 아래로 순서대로 실행된다.
...
Python은 들여쓰기가 문법이다. 탭과 공백을 섞지 않고 일반적으로 공백 4칸을 사용한다.
...
1. Hello와 실행은(는) 무엇인가요?
2. Hello와 실행을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `Hello와 실행`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `Python_List_Range`로 넘어가기 전에 Check Point를 확인한다.
