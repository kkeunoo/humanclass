...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `변수와 연산`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
변수와 연산은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 변수와 연산자는 값을 저장하고 계산하는 기본 도구다. Python의 자료형 변화와 연산 결과를 이해해야 조건문과 반복문으로 이어갈 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 변수 이름으로 값의 의미를 표현하고 계산 전후의 자료형을 `type()`으로 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> `/`와 `//`, `%`는 결과가 다르므로 필요한 계산 목적을 구분한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 문자열과 숫자를 바로 더하려 하면 오류가 발생하므로 입력값 변환이 필요한지 확인한다.
...
1. 변수와 연산은(는) 무엇인가요?
2. 변수와 연산을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `변수와 연산`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
