...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `Tuple`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
| 예상 학습 시간 | 20~35분 |
| 이전 학습 | `Python_String` |
| 다음 학습 | `Python_Variable_Operator` |
...
Tuple은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 튜플은 순서가 있지만 생성 후 항목을 바꾸지 않는 데이터 묶음이다. 리스트와 비교하면 변경 가능성에 따라 자료형을 선택하는 기준을 이해할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 변경하지 않을 값의 묶음은 튜플로 표현하고, 한 개짜리 튜플에는 쉼표가 필요함을 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 튜플은 인덱싱과 반복은 가능하지만 요소를 직접 변경할 수 없다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 괄호만 사용한 `(1)`을 한 개짜리 튜플로 생각하기 쉽지만 실제로는 숫자다.
...
Tuple은 순서가 있지만 생성 후 값을 직접 변경할 수 없다.

```python
single = (1,)
```

값 하나짜리 tuple은 쉼표가 필요하다.
...
➡️ **다음 학습:** `Python_Variable_Operator`
