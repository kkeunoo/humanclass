...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 리스트는 여러 값을 순서대로 저장하고 range는 반복할 숫자 범위를 만든다. 둘을 함께 사용하면 데이터 묶음을 반복 처리할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 리스트 메서드가 원본을 변경하는지, 새 값을 반환하는지 확인하고 range의 끝값이 포함되지 않음을 기억한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 리스트 인덱스는 0부터 시작하고 `range(start, stop)`의 stop은 포함되지 않는다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 존재하지 않는 인덱스에 접근하거나 append의 반환값을 리스트로 착각하지 않는다.
