...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `링크 경로 이미지`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
링크 경로 이미지은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 링크와 이미지는 외부 자원을 연결한다. 상대 경로와 절대 경로를 이해해야 파일 위치가 바뀌어도 깨지지 않는 페이지를 만들 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 프로젝트 내부 파일은 상대 경로를 우선 사용하고, 이미지에는 내용을 설명하는 `alt`를 작성한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> `href`는 이동할 위치, `src`는 불러올 자원의 위치를 나타낸다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 현재 파일의 위치를 기준으로 상대 경로를 계산하지 않아 이미지나 링크가 깨지는 경우가 많다.
...
1. 링크 경로 이미지은(는) 무엇인가요?
2. 링크 경로 이미지을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `링크 경로 이미지`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `HTML_List_Table`로 넘어가기 전에 Check Point를 확인한다.
