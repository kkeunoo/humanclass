...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `문서 기본 구조`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
문서 기본 구조은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 브라우저는 HTML 문서를 위에서 아래로 해석한다. 문서 선언, `html`, `head`, `body`의 역할을 이해해야 CSS와 JavaScript 파일이 어디에서 연결되고 화면 내용이 어디에 작성되는지 판단할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 페이지를 만들기 전에 기본 문서 구조를 먼저 작성하고, `lang`, `charset`, `viewport`, `title`을 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 화면에 보이는 내용은 `body`에, 문서 정보와 외부 파일 연결은 주로 `head`에 둔다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> `head` 안에 화면용 태그를 넣거나 `body` 밖에 요소를 작성하면 브라우저가 보정해 보여 줄 수 있지만 올바른 구조는 아니다.
