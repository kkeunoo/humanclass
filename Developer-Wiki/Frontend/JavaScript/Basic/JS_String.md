...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 웹에서 다루는 사용자 입력과 화면 문구는 대부분 문자열이다. 문자열의 길이, 일부 추출, 검색, 분리를 이해해야 데이터를 화면에 맞게 가공할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 원본 문자열이 직접 변경되는지 새 문자열이 반환되는지 메서드의 반환값을 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 문자열 인덱스는 0부터 시작하며 length는 마지막 인덱스보다 1 크다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 메서드 실행 결과를 변수에 저장하지 않고 원본이 바뀌었다고 생각하는 실수가 많다.
