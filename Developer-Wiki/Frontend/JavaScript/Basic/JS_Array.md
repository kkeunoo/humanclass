...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 배열은 여러 값을 순서대로 관리한다. 같은 종류의 데이터를 변수 여러 개로 나누지 않고 반복문과 함께 처리할 수 있게 한다.

> [!TIP]
> **⭐ 실무 TIP**  
> 추가·삭제 메서드를 사용할 때 원본 배열이 바뀌는지와 반환값이 무엇인지 함께 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 배열의 각 값은 0부터 시작하는 인덱스로 접근한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> `slice`와 `splice`, 요소와 인덱스, 배열 자체와 한 요소를 혼동하지 않는다.
