...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `지도와 외부 API`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
지도와 외부 API은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 외부 API는 다른 서비스가 제공하는 데이터를 정해진 요청 방식으로 사용하게 해 준다. 문서의 주소, 매개변수, 응답 구조를 읽는 능력이 중요하다.

> [!TIP]
> **⭐ 실무 TIP**  
> API를 연결하기 전에 브라우저나 콘솔에서 응답 구조를 먼저 확인하고 필요한 필드만 화면에 사용한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 요청 주소와 응답 데이터의 필드 이름은 API 문서의 규칙을 따라야 한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 응답 구조를 확인하지 않고 예상한 속성명으로 접근하면 undefined가 나온다.
...
1. 지도와 외부 API은(는) 무엇인가요?
2. 지도와 외부 API을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `지도와 외부 API`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `JS_Function_Arrow`로 넘어가기 전에 Check Point를 확인한다.
