...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 이벤트는 사용자의 행동을 JavaScript 실행과 연결하고, 폼 이벤트는 입력값 검증과 제출 흐름을 제어한다.

> [!TIP]
> **⭐ 실무 TIP**  
> 이벤트 처리 함수에서는 어떤 요소에서 어떤 이벤트가 발생했고 기본 동작을 막아야 하는지 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 이벤트 리스너에는 함수 실행 결과가 아니라 실행할 함수 자체를 전달한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> `addEventListener("click", handler())`처럼 괄호를 붙이면 등록 시점에 함수가 실행된다.
