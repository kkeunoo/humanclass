...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `변수와 자료형`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
변수와 자료형은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 변수는 값을 이름으로 저장하고 자료형은 그 값으로 가능한 연산을 결정한다. 자료형을 이해해야 예상하지 못한 문자열 결합이나 비교 오류를 설명할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 재할당이 필요하지 않으면 `const`, 값이 바뀌어야 할 때만 `let`을 사용한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 콘솔에서 값뿐 아니라 `typeof`도 함께 확인하면 자료형 문제를 빠르게 찾을 수 있다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 입력 요소의 value는 숫자처럼 보여도 문자열이므로 계산 전에 변환이 필요한지 확인한다.
...
1. 변수와 자료형은(는) 무엇인가요?
2. 변수와 자료형을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `변수와 자료형`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
