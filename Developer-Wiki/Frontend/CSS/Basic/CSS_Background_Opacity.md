...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `배경과 투명도`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
배경과 투명도은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 배경은 콘텐츠와 분리된 장식 영역이며 opacity는 요소 전체의 투명도에 영향을 준다. 둘의 차이를 알아야 글자까지 흐려지는 문제를 피할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 배경만 투명하게 만들고 싶다면 요소 전체의 `opacity`보다 rgba 색상이나 배경색의 알파값을 사용한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> `opacity`는 선택한 요소와 그 자식 전체에 적용된다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 부모에 opacity를 주고 자식 글자만 다시 선명하게 만들려고 해도 독립적으로 되돌리기 어렵다.
