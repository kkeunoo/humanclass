...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `선택자와 우선순위`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.

| 항목 | 내용 |
|---|---|
| 난이도 | ★★☆☆☆ |
| 예상 학습 시간 | 20~35분 |
| 이전 학습 | `CSS_Position_Overflow` |
| 다음 학습 | `CSS_Transition_Transform` |
...
선택자와 우선순위은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> CSS는 먼저 “어떤 요소에 적용할 것인가”를 찾아야 한다. 선택자와 우선순위를 이해해야 스타일이 적용되지 않는 원인을 추적할 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 가능하면 재사용 가능한 클래스 선택자를 중심으로 작성하고, 지나치게 긴 후손 선택자는 피한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> 같은 속성이 충돌하면 선택자의 구체성, 선언 순서, 상속 여부를 차례로 확인한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 스타일이 적용되지 않을 때 무조건 `!important`를 추가하면 이후 수정이 더 어려워진다.
...
1. 선택자와 우선순위은(는) 무엇인가요?
2. 선택자와 우선순위을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `선택자와 우선순위`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `CSS_Transition_Transform`로 넘어가기 전에 Check Point를 확인한다.
...
➡️ **다음 학습:** `CSS_Transition_Transform`
