...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `Position과 Overflow`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.

| 항목 | 내용 |
|---|---|
| 난이도 | ★★★☆☆ |
| 예상 학습 시간 | 35~50분 |
| 이전 학습 | `CSS_Media_Query` |
| 다음 학습 | `CSS_Selector` |
...
- `Position과 Overflow`이 무엇인지 자신의 말로 설명한다.
...
Position과 Overflow은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 요소를 일반 문서 흐름에서 이동하거나 특정 위치에 고정하려면 기준점과 겹침을 이해해야 한다. overflow는 넘친 콘텐츠를 어떻게 처리할지 결정한다.

> [!TIP]
> **⭐ 실무 TIP**  
> absolute 요소를 배치할 때는 기준이 될 부모에 `position: relative`가 필요한지 먼저 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> absolute는 가장 가까운 위치 지정 조상을 기준으로 삼고, fixed는 화면을 기준으로 한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 위치가 맞지 않을 때 top과 left 값만 바꾸기보다 기준 요소가 누구인지 먼저 확인한다.
...
1. Position과 Overflow은(는) 무엇인가요?
2. Position과 Overflow을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `Position과 Overflow`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `CSS_Selector`로 넘어가기 전에 Check Point를 확인한다.
