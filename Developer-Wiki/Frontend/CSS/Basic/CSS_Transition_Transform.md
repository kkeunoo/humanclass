...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `Transition과 Transform`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.
...
- `Transition과 Transform`이 무엇인지 자신의 말로 설명한다.
...
Transition과 Transform은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> transform은 요소의 모양과 위치를 시각적으로 바꾸고 transition은 그 변화를 시간에 따라 이어 준다. 둘을 함께 이해해야 자연스러운 hover 효과를 만들 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 애니메이션할 속성을 명확히 지정하고 `transition: all`을 습관적으로 사용하지 않는다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> transform으로 이동한 요소는 문서의 원래 자리 계산 자체를 바꾸지 않는다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> hover 상태에만 transition을 선언하면 마우스가 빠질 때 애니메이션이 다르게 보일 수 있다.
...
1. Transition과 Transform은(는) 무엇인가요?
2. Transition과 Transform을(를) 사용하는 이유는 무엇인가요?
3. 이 문서의 기본 예제를 말로 설명해 보세요.
4. 학습 중 가장 자주 발생할 수 있는 실수는 무엇인가요?

# 최종 요약

- `Transition과 Transform`의 이름만 외우지 않고 필요한 이유와 동작 순서를 함께 이해한다.
- 작은 예제를 실행하고 값을 바꾸면서 결과를 비교한다.
- 문제를 풀 때 입력 → 처리 → 출력으로 나눈다.
- 오류 메시지와 중간값을 확인하는 습관을 만든다.
- 다음 문서 `다음 파트 README`로 넘어가기 전에 Check Point를 확인한다.
