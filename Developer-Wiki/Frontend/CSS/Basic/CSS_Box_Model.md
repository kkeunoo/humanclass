...
> [!IMPORTANT]
> **핵심 목표**  
> 이 문서는 수업 범위 안에서 `박스 모델`의 개념, 사용 이유, 기본 문법, 예제, 실수 사례와 복습 질문을 한 번에 학습하도록 구성한다.

| 항목 | 내용 |
|---|---|
| 난이도 | ★★☆☆☆ |
| 예상 학습 시간 | 20~35분 |
| 이전 학습 | `CSS_Background_Opacity` |
| 다음 학습 | `CSS_Display` |
...
박스 모델은(는) 이후 예제와 문제를 이해하기 위한 기본 도구다. 문법만 외우기보다 어떤 상황에서 필요하고, 코드가 어떤 순서로 동작하는지 이해해야 다른 문제에도 적용할 수 있다.
...
> [!NOTE]
> **💡 WHY — 왜 중요한가?**  
> 브라우저는 모든 요소를 콘텐츠, 안쪽 여백, 테두리, 바깥 여백을 가진 상자로 계산한다. 이 계산을 이해해야 예상한 크기와 간격을 만들 수 있다.

> [!TIP]
> **⭐ 실무 TIP**  
> 전체 크기를 직관적으로 관리하려면 프로젝트 초기에 `box-sizing: border-box` 적용 여부를 확인한다.

> [!IMPORTANT]
> **📌 반드시 기억하기**  
> `width`만 보고 실제 요소 너비를 판단하지 말고 padding과 border가 포함되는 방식을 확인한다.

> [!WARNING]
> **⚠️ 주제별 자주 하는 실수**  
> 세로 margin이 기대와 다르게 보일 때는 인접한 블록 요소의 margin 관계를 확인한다.
