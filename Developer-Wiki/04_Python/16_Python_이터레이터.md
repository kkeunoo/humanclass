...
- [학습 목표](#py-16-section-4)
- [개념에서 실제 실행까지 — 이터레이터란? — 다음 값과 소진 상태](#py-16-section-5)
- [5. 반복 가능한 객체와 이터레이터 비교](#py-16-section-10)
- [25. 이터레이터와 리스트 비교](#py-16-section-30)
- [32. 핵심 요약](#py-16-section-37)
- [33. 최종 체크리스트](#py-16-section-38)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-16-section-1)
- [개요](#py-16-section-2)
- [핵심 개념](#py-16-section-3)
- [학습 목표](#py-16-section-4)
- [개념에서 실제 실행까지 — 이터레이터란? — 다음 값과 소진 상태](#py-16-section-5)
- [1. 반복 가능한 객체란?](#py-16-section-6)
- [2. 이터레이터란?](#py-16-section-7)
- [3. `StopIteration`](#py-16-section-8)
- [4. `for`문의 내부 동작](#py-16-section-9)
- [5. 반복 가능한 객체와 이터레이터 비교](#py-16-section-10)
- [6. 이터레이터는 현재 위치를 기억한다](#py-16-section-11)
- [7. 이터레이터는 한 번 소비된다](#py-16-section-12)
- [8. 리스트는 여러 번 반복할 수 있는 이유](#py-16-section-13)
- [9. 문자열 이터레이터](#py-16-section-14)
- [10. 딕셔너리 이터레이터](#py-16-section-15)
- [11. `range`와 이터레이터](#py-16-section-16)
- [12. `iter()`에 종료값 전달](#py-16-section-17)
- [13. 이터레이션 프로토콜](#py-16-section-18)
- [14. 사용자 정의 이터레이터](#py-16-section-19)
- [15. 사용자 정의 이터레이터의 동작 과정](#py-16-section-20)
- [16. 사용자 정의 이터레이터를 `for`문에서 사용](#py-16-section-21)
- [17. 반복 가능한 객체와 이터레이터를 분리하기](#py-16-section-22)
- [18. 역순 이터레이터 예제](#py-16-section-23)
- [19. 무한 이터레이터](#py-16-section-24)
- [20. `next()`의 기본값](#py-16-section-25)
- [21. 파일 객체도 이터레이터다](#py-16-section-26)
- [22. 이터레이터를 사용하는 대표 기능](#py-16-section-27)
- [23. `map()`도 이터레이터를 반환한다](#py-16-section-28)
- [24. 이터레이터와 메모리](#py-16-section-29)
- [25. 이터레이터와 리스트 비교](#py-16-section-30)
- [26. 이터레이터와 제너레이터](#py-16-section-31)
- [27. 기존 방식에서 개선된 이해](#py-16-section-32)
- [28. 대표 오류로 이해하기](#py-16-section-33)
- [29. 이터레이터 구조](#py-16-section-34)
- [30. 자주 하는 실수](#py-16-section-35)
- [31. 면접·복습 포인트](#py-16-section-36)
- [32. 핵심 요약](#py-16-section-37)
- [33. 최종 체크리스트](#py-16-section-38)
- [마무리](#py-16-section-39)
- [V3 동작 백과 보강 — `iter()`와 `next()`의 실제 계약](#py-16-section-40)

</details>

---

<a id="py-16-section-1"></a>
...
| 분류 | `04_Python` |
| 권장 선수 학습 | `04_Python_리스트와_데이터처리.md`, `09_Python_반복문.md`, `11_Python_함수.md`, `12_Python_클래스.md` |
...
<a id="py-16-section-4"></a>

## 학습 목표

- 반복 가능 객체와 일회 소비되는 상태를 구분한다.
- 예제의 입력·처리·출력과 대표 실패 조건을 직접 확인한다.

---

<a id="py-16-section-5"></a>

## 개념에서 실제 실행까지 — 이터레이터란? — 다음 값과 소진 상태

### 무엇이며 왜 배워야 할까?

이터러블은 iter로 이터레이터를 얻을 수 있는 객체이고 이터레이터는 next로 다음 값을 요청하는 객체다. 리스트는 반복할 때마다 새 이터레이터를 얻을 수 있지만 이미 만들어진 이터레이터 자체는 소비 위치를 유지한다. 마지막 다음 값을 요청하면 StopIteration이 발생한다. for는 이 반복 프로토콜을 이용해 종료를 처리한다.

수업에서는 04번의 map·reversed, 09번의 for가 실제 소비 사례다. map 자체는 최종 결과 목록이 아니며 list(map_obj)가 순회하여 목록을 만든다. 같은 map을 다시 목록으로 만들면 이미 소진되어 []가 될 수 있다. ‘값이 사라졌다’가 아니라 ‘그 반복 상태를 끝까지 소비했다’는 뜻이다.

💡 16번에 대응하는 별도 원본 구현 파일은 없다. 14_try.py 하단의 후속 주제 메모 및 앞선 반복 코드와 연결한 확장 학습 문서다. 아래는 그 동작을 독립적으로 재현하는 보충 예제이며 수업 원본에 있었다고 주장하지 않는다.
...
1. numbers는 재순회 가능한 목록이고 iter(numbers)는 소비 위치가 있는 별도 객체다.
2. next가 첫 값 10을 반환하고 소비 위치를 이동한다.
3. list(it)는 남은 20만 목록으로 모은다. 같은 it를 다시 소비하면 []다.
4. numbers 자체는 그대로이며 소진한 it의 다음 요청은 StopIteration이다.

### 원본에서 어디에 사용했을까?


#### 내 코드: `14_try.py` 154~160행
...
다시 순회하려면 원본 목록에서 iter를 다시 얻는다. 스트림이나 제너레이터는 원본 자체도 일회 소비일 수 있어 재사용 계약을 확인한다.
...
### 수업 map과 연결 — 변환은 소비 시점에 진행

map은 변환 함수와 반복 대상을 연결한 지연 반복 결과다. 이 예제의 transform 출력으로 호출 시점이 아니라 next/list 소비 시점에 함수가 실행됨을 관찰한다.

```python
def transform(value):
    print('변환', value)
    return int(value)
result = map(transform, ['1', '2'])
print('map 생성')
print(next(result))
print(list(result))
print(list(result))
...
1. next(it) 다음에 list(it)를 먼저 출력하면 왜 10이 다시 나오지 않을까?
2. 같은 map을 list로 두 번 만들면 두 번째 결과는 왜 빈 목록인가?

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. next가 이미 10을 소비하여 위치가 두 번째 요소로 이동했기 때문이다. list는 남은 값만 소비한다.
2. 첫 소비가 이터레이터의 값을 소진했기 때문이다. 재사용하려면 원본에서 새 map을 만들거나 첫 결과를 저장한다.
