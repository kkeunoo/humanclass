...
- [학습 목표](#py-14-section-4)
- [개념에서 실제 실행까지 — 예외 처리란? — try·except·else·finally의 경로](#py-14-section-5)
- [15. 예외 객체와 문자열 비교 오류](#py-14-section-20)
- [29. 핵심 요약](#py-14-section-34)
- [30. 최종 체크리스트](#py-14-section-35)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-14-section-1)
- [개요](#py-14-section-2)
- [핵심 개념](#py-14-section-3)
- [학습 목표](#py-14-section-4)
- [개념에서 실제 실행까지 — 예외 처리란? — try·except·else·finally의 경로](#py-14-section-5)
- [1. 예외 처리 전: 조건문으로 오류 방지](#py-14-section-6)
- [2. `try`와 `except` 기본 구조](#py-14-section-7)
- [3. 빈 `except`의 문제점](#py-14-section-8)
- [4. 예외 종류별 처리](#py-14-section-9)
- [5. 여러 `except`의 순서](#py-14-section-10)
- [6. `as e`로 예외 객체 확인](#py-14-section-11)
- [7. `Exception`으로 일반 예외 처리](#py-14-section-12)
- [8. `else`](#py-14-section-13)
- [9. `finally`](#py-14-section-14)
- [10. `finally`는 언제 사용할까?](#py-14-section-15)
- [11. `try`·`except`·`else`·`finally` 실행 순서](#py-14-section-16)
- [12. `raise`](#py-14-section-17)
- [13. 반환 코드 방식의 로그인 검증](#py-14-section-18)
- [14. `raise`를 이용한 로그인 검증](#py-14-section-19)
- [15. 예외 객체와 문자열 비교 오류](#py-14-section-20)
- [16. 내장 예외 종류를 의미에 맞게 사용하기](#py-14-section-21)
- [17. 사용자 정의 예외](#py-14-section-22)
- [18. `traceback`](#py-14-section-23)
- [19. `traceback`은 사용자 화면에 보여줘야 할까?](#py-14-section-24)
- [20. 예외를 처리한 뒤 다시 발생시키기](#py-14-section-25)
- [21. 예외 처리 범위는 작게 유지하기](#py-14-section-26)
- [22. 예외를 무시해도 되는 경우](#py-14-section-27)
- [23. 예외 처리와 반환값](#py-14-section-28)
- [24. 대표 오류로 이해하기](#py-14-section-29)
- [25. 예외 처리 구조](#py-14-section-30)
- [26. 기존 코드에서 개선 코드로 바꾼 이유](#py-14-section-31)
- [27. 자주 하는 실수](#py-14-section-32)
- [28. 면접·복습 포인트](#py-14-section-33)
- [29. 핵심 요약](#py-14-section-34)
- [30. 최종 체크리스트](#py-14-section-35)
- [마무리](#py-14-section-36)
- [V3 동작 백과 보강 — 예외 처리의 정확한 실행 순서](#py-14-section-37)

</details>

---

<a id="py-14-section-1"></a>
...
| 분류 | `04_Python` |
| 권장 선수 학습 | `08_Python_조건문과_패턴매칭.md`, `11_Python_함수.md`, `12_Python_클래스.md`, `13_Python_상속과_다형성.md` |
...
| `except` | 발생한 예외를 처리 |
| `else` | try가 예외·return 등의 탈출 없이 끝난 경우 실행 |
| `finally` | 정상·예외·반환 경로에서 수행하는 정리 작업 |
...
<a id="py-14-section-4"></a>

## 학습 목표

- 실패·성공·반환 경로와 정리 작업을 구분한다.
- 예제의 입력·처리·출력과 대표 실패 조건을 직접 확인한다.

---

<a id="py-14-section-5"></a>

## 개념에서 실제 실행까지 — 예외 처리란? — try·except·else·finally의 경로

### 무엇이며 왜 배워야 할까?

예외 처리는 정상 계산이 실패했을 때 수행할 경로를 지정하는 기능이다. try를 실행하다 예외가 발생하면 일치하는 except로 이동한다. else는 try가 예외 없이 끝나고 return·break 등의 탈출도 하지 않았을 때 실행된다. finally는 보통 성공·예외·반환 경로 모두에서 정리 작업을 수행한다. 프로세스 강제 종료 같은 모든 상황까지 실행이 보장되는 것은 아니다.

원본 div7은 try 안에 return이 있어 성공 계산이 끝나자 반환 경로로 들어간다. 이때 else는 건너뛰고 finally를 실행한 후 호출자에게 돌아간다. ‘try 성공이면 else가 무조건 실행된다’로 외우면 출력 순서를 잘못 예상한다.

내 초기 div는 0으로 나눌 때 result를 대입하지 못한 뒤 반환하여 UnboundLocalError를 만들 수 있다. 강사님은 result를 0으로 초기화해 그 오류를 피하지만 실패를 0이라는 정상 값과 구분할 수 있는지 별도로 판단해야 한다.

except:는 KeyboardInterrupt·SystemExit 같은 BaseException 계열도 잡을 수 있다. 일반적인 운영 오류는 구체적 예외나 Exception 범위로 다루며 무조건 pass로 숨기지 않는다. 예외 객체와 문자열 'code:2'를 직접 비교하는 것은 오류 코드를 검사하는 방법이 아니다.
...
def divide(a, b):
    try:
        ...
        print('finally')
print('결과', divide(10, 2))
print('결과', divide(10, 0))
```
...
1. divide(10,2)의 try에서 계산하고 바로 return 경로로 들어간다.
2. finally를 실행한 뒤 5.0을 호출자에게 전달한다. else는 실행하지 않는다.
3. divide(10,0)에서는 계산 중 ZeroDivisionError가 발생해 except로 이동한다.
4. except의 None 반환도 finally를 거친 뒤 호출자에게 전달된다.
...
정상 반환과 실패 반환을 호출자가 구분하도록 계약을 적는다. finally의 return은 앞선 반환이나 예외를 덮을 수 있어 정리 작업에서 사용하지 않는 편이 안전하다.
...
| try에서 생긴 일 | except | else | finally | 이후 |
| --- | --- | --- | --- | --- |
| 끝까지 정상 실행 | 건너뜀 | 실행 | 실행 | 다음 문장 |
| 처리되는 예외 | 해당 블록 | 건너뜀 | 실행 | 다음 문장/반환 |
| try 내부 return | 건너뜀 | 건너뜀 | 실행 | 호출자에게 반환 |
| 처리되지 않는 예외 | 없음 | 건너뜀 | 실행 | 호출 경로로 전달 |

이 표는 프로세스가 정상적으로 제어 흐름을 수행하는 경우다. finally에서 새 예외나 return을 만들면 기존 결과를 바꿀 수 있다.
...
1. try에서 return을 지우고 result=a/b로 바꾸면 성공 시 else는?
2. except Exception은 Ctrl+C의 KeyboardInterrupt까지 잡는가?

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. 정상적으로 try 끝까지 도달하므로 else가 실행된다. 함수 마지막에 결과 반환을 별도로 설계해야 한다.
2. 보통 잡지 않는다. KeyboardInterrupt는 BaseException의 직접 계열이다. bare except와 범위가 다르다.
...
<a id="py-14-section-37"></a>

## V3 동작 백과 보강 — 예외 처리의 정확한 실행 순서

먼저 `try`를 실행한다. 예외가 생기면 남은 `try` 문장을 건너뛰고 가장 먼저 일치하는 `except`로 이동한다. 예외가 없고 `try`에서 `return` 등으로 탈출하지 않을 때 `else`가 실행되며, `finally`는 정상·예외·`return` 여부와 관계없이 정리 작업을 수행한다. 처리하지 않은 예외는 호출자에게 전달된다.
