...
- [실습 진행 방법](#py-20-section-3)
- [개념에서 실제 실행까지 — 종합실습이란? — 주문 문자열을 검증된 매출로](#py-20-section-6)
- [16. 실습 파일을 직접 만들 때 권장 방식](#py-20-section-22)
- [18. 핵심 요약](#py-20-section-24)
- [19. 최종 체크리스트](#py-20-section-25)
- [V3 동작 백과 보강 — 종합실습 실행 기록법](#py-20-section-27)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-20-section-1)
- [개요](#py-20-section-2)
- [실습 진행 방법](#py-20-section-3)
- [난이도 구성](#py-20-section-4)
- [공통 코딩 기준](#py-20-section-5)
- [개념에서 실제 실행까지 — 종합실습이란? — 주문 문자열을 검증된 매출로](#py-20-section-6)
- [1. 사용자 목록 분석](#py-20-section-7)
- [2. 학생 성적 관리](#py-20-section-8)
- [3. 상품 재고 관리](#py-20-section-9)
- [4. 카페 주문 관리](#py-20-section-10)
- [5. 은행 계좌 클래스](#py-20-section-11)
- [6. 회원 등록과 JSON 저장](#py-20-section-12)
- [7. 로그 분석기](#py-20-section-13)
- [8. 최종 프로젝트: HumanJobs 직원 관리 시스템](#py-20-section-14)
- [9. 직원 데이터 모델](#py-20-section-15)
- [10. 직원 관리자 클래스](#py-20-section-16)
- [11. 파일 저장과 불러오기](#py-20-section-17)
- [12. 프로젝트 실행 예시](#py-20-section-18)
- [13. 최종 프로젝트 구조 분리](#py-20-section-19)
- [14. 최종 프로젝트 확장 과제](#py-20-section-20)
- [15. 리팩토링 체크 과정](#py-20-section-21)
- [16. 실습 파일을 직접 만들 때 권장 방식](#py-20-section-22)
- [17. 자주 하는 실수](#py-20-section-23)
- [18. 핵심 요약](#py-20-section-24)
- [19. 최종 체크리스트](#py-20-section-25)
- [마무리](#py-20-section-26)
- [V3 동작 백과 보강 — 종합실습 실행 기록법](#py-20-section-27)
...
<a id="py-20-section-6"></a>

## 개념에서 실제 실행까지 — 종합실습이란? — 주문 문자열을 검증된 매출로

### 무엇이며 왜 배워야 할까?

주문 실습은 텍스트 줄→필드 분리→필드별 공백 제거→숫자 변환→유효 범위 검사→매출 누적으로 이어진다. 각 단계의 자료형과 실패 조건을 기록하면 결과가 틀렸을 때 어느 단계가 잘못되었는지 찾기 쉽다.

내 exam/99_try.py는 각 필드에 strip을 적용하고 최종 991_try.py는 줄 전체에 strip 후 split한다. 뒤 방식만으로 상품명 끝·각 필드 앞 공백이 모두 제거되지 않는다. int는 일부 공백을 허용하므로 숫자는 정상 변환되어 이 차이를 감출 수 있다. 마지막 줄바꿈에서 생긴 빈 문자열을 orders[2]로 접근하면 IndexError가 난다.

원본 조건은 수량 0 이상, 단가 0~10000이다. 개선 과정에서 >0으로 바꾸면 요구사항이 달라진다. 가격의 원을 제거할 때 가격 필드만 대상으로 삼는 판단은 유지한다.

💡 아래는 파일을 덮어쓰지 않도록 같은 형식의 입력을 문자열로 넣은 보충 구현이다. 따옴표 안 쉼표까지 지원하려면 csv 모듈 등의 확장이 필요하다. 뒤의 기존 난이도별 종합 프로젝트는 삭제하지 않고 유지했다.
...
for line_number, line in enumerate(text.splitlines(), start=1):
    ...
    if len(fields) != 3:
        rejected.append((line_number, '필드 수'))
        continue
    name, quantity_text, price_text = fields
    try:
        quantity = int(quantity_text)
        price = int(price_text.removesuffix('원').strip())
    except ValueError:
        rejected.append((line_number, '숫자 형식'))
        continue
    if quantity < 0 or not 0 <= price <= 10000:
        rejected.append((line_number, '범위'))
        continue
    accepted.append((name, quantity, price))
...
1. 입력 텍스트를 줄로 나누고 줄 번호와 함께 순회한다.
2. 빈 줄은 건너뛰고 세 필드 여부를 검사한 뒤 필드별 공백을 정리한다.
3. 수량·단가를 int로 바꾸고 원본 범위 조건을 검사한다. 실패하면 이유를 기록하고 누적 전에 continue한다.
4. 유효 주문만 accepted에 추가하고 수량×단가를 total에 누적하여 결과와 거절 사유를 출력한다.

### 원본에서 어디에 사용했을까?


#### 내 코드: `991_try.py` 31~37행

문맥 확인용 원본 발췌다. 이 조각만 독립 실행할 수 있다는 뜻은 아니다. 주석의 설명은 아래 실제 동작 해설과 대조한다.

```python
        isBlank = orders.strip().split(',')
        orders = isBlank
        print(orders)

        # 조건 3-2_가격에서 '원' 제거
        # 사용자의 입장에서 생각. '원'이 다른곳에도 있다면? 다 바뀌니까 2번 index 가격란만 변경
        isOne = orders[2].replace('원','')
```

이 주제의 같은 이름 대응 파일은 강사님 원본에 없다. 관련 수업 개념과 아래 확장 예제를 구분하며 존재하지 않는 비교 구현을 만들지 않는다.

보충 입력 매출은 13500이다. 원본 문제의 네 유효 주문에서는 6000+4500+1000+7500=19000이다. 서로 다른 입력의 결과를 섞지 않는다.
...
수량·단가의 허용 범위는 원본 요구사항을 그대로 따른다. 0을 허용하는 것과 음수를 제거하는 것은 다르다. 다음 결과로 범위 검사를 누적 계산과 분리하여 확인한다.

```python
for quantity, price in [(0, 0), (1, 10000), (1, 10001), (-1, 3000)]:
    valid = quantity >= 0 and 0 <= price <= 10000
    print(quantity, price, valid)
```
...
1. 무료, 0, 0원과 상품, 1, 10001은 수락될까?
2. 끝의 빈 줄·필드 누락·숫자 아닌 수량·음수 가격에 각각 대응하라.

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. 첫째는 원본 조건에 맞아 수락되고 매출 0이다. 둘째는 단가 상한을 넘어 거절된다.
2. 빈 줄은 건너뛰고 필드 수를 먼저 검사한다. 숫자 형식은 ValueError 처리, 음수 단가는 범위 검사로 거절한다. 실패 이유와 줄 번호를 기록한다.
