...
- [학습 목표](#py-17-section-4)
- [개념에서 실제 실행까지 — 제너레이터란? — 호출, 일시정지, 재개](#py-17-section-5)
- [10. 사용자 정의 이터레이터와 제너레이터 비교](#py-17-section-15)
- [12. 리스트 컴프리헨션과 제너레이터 표현식 비교](#py-17-section-17)
- [36. 핵심 요약](#py-17-section-41)
- [37. 최종 체크리스트](#py-17-section-42)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-17-section-1)
- [개요](#py-17-section-2)
- [핵심 개념](#py-17-section-3)
- [학습 목표](#py-17-section-4)
- [개념에서 실제 실행까지 — 제너레이터란? — 호출, 일시정지, 재개](#py-17-section-5)
- [1. 일반 함수와 제너레이터 함수](#py-17-section-6)
- [2. `yield`란?](#py-17-section-7)
- [3. 제너레이터 함수는 호출 즉시 실행되지 않는다](#py-17-section-8)
- [4. 실행 위치와 지역 변수 유지](#py-17-section-9)
- [5. 제너레이터 종료와 `StopIteration`](#py-17-section-10)
- [6. `for`문에서 제너레이터 사용](#py-17-section-11)
- [7. `return`과 `yield` 차이](#py-17-section-12)
- [8. 제너레이터 안의 `return`](#py-17-section-13)
- [9. `yield`를 반복문과 함께 사용](#py-17-section-14)
- [10. 사용자 정의 이터레이터와 제너레이터 비교](#py-17-section-15)
- [11. 제너레이터 표현식](#py-17-section-16)
- [12. 리스트 컴프리헨션과 제너레이터 표현식 비교](#py-17-section-17)
- [13. 제너레이터는 한 번 소비된다](#py-17-section-18)
- [14. `yield from`](#py-17-section-19)
- [15. 여러 제너레이터 연결](#py-17-section-20)
- [16. 무한 제너레이터](#py-17-section-21)
- [17. 제너레이터로 파일 한 줄씩 처리](#py-17-section-22)
- [18. 데이터 필터링 제너레이터](#py-17-section-23)
- [19. 데이터 변환 제너레이터](#py-17-section-24)
- [20. 제너레이터 파이프라인](#py-17-section-25)
- [21. `send()`](#py-17-section-26)
- [22. `close()`](#py-17-section-27)
- [23. `GeneratorExit`](#py-17-section-28)
- [24. `throw()`](#py-17-section-29)
- [25. 전통적인 코루틴과 `send()`](#py-17-section-30)
- [26. 제너레이터와 메모리](#py-17-section-31)
- [27. 제너레이터의 장점](#py-17-section-32)
- [28. 제너레이터의 단점](#py-17-section-33)
- [29. 제너레이터가 적합한 상황](#py-17-section-34)
- [30. 제너레이터가 적합하지 않은 상황](#py-17-section-35)
- [31. 대표 오류로 이해하기](#py-17-section-36)
- [32. 제너레이터 구조](#py-17-section-37)
- [33. 기존 방식에서 개선된 이해](#py-17-section-38)
- [34. 자주 하는 실수](#py-17-section-39)
- [35. 면접·복습 포인트](#py-17-section-40)
- [36. 핵심 요약](#py-17-section-41)
- [37. 최종 체크리스트](#py-17-section-42)
- [마무리](#py-17-section-43)
- [V3 동작 백과 보강 — 호출과 실행이 분리되는 함수](#py-17-section-44)

</details>

---

<a id="py-17-section-1"></a>
...
<a id="py-17-section-4"></a>

## 학습 목표

- yield 앞뒤의 실행 시점과 소비를 추적한다.
- 예제의 입력·처리·출력과 대표 실패 조건을 직접 확인한다.

---

<a id="py-17-section-5"></a>

## 개념에서 실제 실행까지 — 제너레이터란? — 호출, 일시정지, 재개

### 무엇이며 왜 배워야 할까?

제너레이터는 필요할 때 값을 하나씩 만드는 이터레이터다. yield가 있는 함수를 호출하면 즉시 몸체의 모든 계산을 수행하지 않고 제너레이터 객체를 만든다. next나 for로 값을 요구할 때 실행이 시작되고 yield에서 값을 내보낸 뒤 실행 상태를 보존하여 멈춘다. 다음 요청에서 그 뒤부터 재개한다.

return은 일반적으로 호출을 끝내지만 yield는 반복 값을 내보내고 잠시 멈춘다. 다음 예제에서 '시작' 출력이 g=produce()가 아니라 next(g)에서 나오는 이유가 이 지연 실행이다. 마지막 list(g)는 남은 값을 소비하고, 한 번 더 소비하면 빈 결과다.

💡 대응하는 17번 수업 소스 파일은 없다. 14번 하단 메모의 후속 주제와 함수·반복 학습을 연결한 보충 학습이다. 큰 입력을 전부 리스트로 만들지 않고 순차 처리할 때 유용하지만 모든 처리의 메모리가 무조건 작아지는 것은 아니다. 제너레이터 안에 거대한 자료를 따로 저장하면 그 비용은 남는다.
...
1. produce() 호출은 몸체를 아직 실행하지 않고 g를 만든다.
2. 객체 생성 완료를 출력한 뒤 next(g)에서 몸체를 시작해 시작과 10을 출력한다.
3. list(g) 소비가 yield 다음부터 재개하여 재개를 출력하고 20을 모은다.
4. 다시 list(g)는 이미 종료한 제너레이터에서 더 얻을 값이 없어 []다.
...
예외도 함수 호출 시점이 아니라 해당 코드를 실제 소비하는 시점에 발생할 수 있다. 디버깅할 때 생성 위치와 소비 위치를 함께 찾는다.
...
1. 두 yield를 return [10,20]으로 바꾸면 함수 호출 시점이 어떻게 달라질까?
2. 제너레이터 함수 호출 직후 몸체에서 발생할 예외가 항상 즉시 드러나는가?

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. 몸체가 호출 즉시 실행되어 리스트를 반환한다. yield에 의한 중단·재개 상태는 사라진다.
2. 아니다. 몸체가 소비 시점에 실행되어 next나 for 위치에서 예외가 드러날 수 있다.
...
<a id="py-17-section-16"></a>

## 11. 제너레이터 표현식

리스트 컴프리헨션과 비슷한 문법으로 제너레이터를 만들 수 있다.

<a id="index-section-49"></a>
