...
- [학습 목표](#py-03-section-4)
- [개념에서 실제 실행까지 — 문자열 처리란? — 원본, 반환 문자열, 분리된 목록](#py-03-section-5)
- [10. 문자열 포매팅 방식 비교](#py-03-section-15)
- [33. 내 코드와 강사님 코드 비교](#py-03-section-38)
- [38. 핵심 요약](#py-03-section-43)
- [39. 최종 체크리스트](#py-03-section-44)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-03-section-1)
- [개요](#py-03-section-2)
- [핵심 개념](#py-03-section-3)
- [학습 목표](#py-03-section-4)
- [개념에서 실제 실행까지 — 문자열 처리란? — 원본, 반환 문자열, 분리된 목록](#py-03-section-5)
- [1. 작은따옴표와 큰따옴표](#py-03-section-6)
- [2. 여러 줄 문자열](#py-03-section-7)
- [3. 이스케이프 문자](#py-03-section-8)
- [4. 문자열과 숫자 연결 오류](#py-03-section-9)
- [5. f-string](#py-03-section-10)
- [6. f-string 안의 표현식](#py-03-section-11)
- [7. `format()` 메서드](#py-03-section-12)
- [8. 여러 줄 f-string](#py-03-section-13)
- [9. `%` 문자열 포매팅](#py-03-section-14)
- [10. 문자열 포매팅 방식 비교](#py-03-section-15)
- [11. `len()`으로 문자열 길이 확인](#py-03-section-16)
- [12. `count()`](#py-03-section-17)
- [13. `find()`](#py-03-section-18)
- [14. `index()`](#py-03-section-19)
- [15. `rfind()`](#py-03-section-20)
- [16. `replace()`](#py-03-section-21)
- [17. 문자열은 변경 불가능하다](#py-03-section-22)
- [18. `split()`](#py-03-section-23)
- [19. 리스트 언패킹](#py-03-section-24)
- [20. `join()`](#py-03-section-25)
- [21. `split()`과 `join()` 연결](#py-03-section-26)
- [22. 대소문자 구분 검색](#py-03-section-27)
- [23. `upper()`와 `lower()`](#py-03-section-28)
- [24. `casefold()`](#py-03-section-29)
- [25. `strip()`](#py-03-section-30)
- [26. 모든 공백 제거](#py-03-section-31)
- [27. `lstrip()`과 `rstrip()`](#py-03-section-32)
- [28. `zfill()`](#py-03-section-33)
- [29. f-string 자리수 지정](#py-03-section-34)
- [30. f-string 정렬](#py-03-section-35)
- [31. 실수 포맷](#py-03-section-36)
- [32. 천 단위 구분 기호](#py-03-section-37)
- [33. 내 코드와 강사님 코드 비교](#py-03-section-38)
- [34. 기존 코드에서 개선 코드로 바꾼 이유](#py-03-section-39)
- [35. 실무형 예제: 사용자 정보 요약](#py-03-section-40)
- [36. 대표 오류로 이해하기](#py-03-section-41)
- [37. 자주 하는 실수](#py-03-section-42)
- [38. 핵심 요약](#py-03-section-43)
- [39. 최종 체크리스트](#py-03-section-44)
- [마무리](#py-03-section-45)
- [V3 동작 백과 보강 — 문자열 객체가 처리되는 방식](#py-03-section-46)

</details>

---

<a id="py-03-section-1"></a>
...
<a id="py-03-section-4"></a>

## 학습 목표

- 문자열 메서드의 입력·반환·실패 조건을 설명한다.
- 예제의 입력·처리·출력과 대표 실패 조건을 직접 확인한다.

---

<a id="py-03-section-5"></a>

## 개념에서 실제 실행까지 — 문자열 처리란? — 원본, 반환 문자열, 분리된 목록

### 무엇이며 왜 배워야 할까?

문자열은 텍스트를 저장하는 불변 객체다. strip, replace, lower는 문자열 내용을 직접 바꾸지 않고 결과 문자열을 반환한다. 반환값을 저장하지 않으면 이후 코드에서 원래 값이 계속 보일 수 있다. 메서드 호출 왼쪽의 객체가 str인지 먼저 확인한다.

공백 정리는 위치에 따라 다르다. strip은 양끝 공백을 제거하고 내부 공백은 유지한다. split()은 연속 공백을 구분자로 묶어 처리하지만 split(' ')은 하나의 공백을 정확히 구분자로 삼아 빈 항목을 만들 수 있다. join은 구분자 문자열의 메서드이며 인수로 전달한 요소들이 문자열이어야 한다.

find는 검색 실패에 -1을 반환하고 index는 ValueError를 발생시킨다. find 결과 0은 첫 위치를 찾았다는 뜻이고 -1도 bool(-1)은 True이므로 if text.find(...):는 검색 여부 판단에 부적절하다. >= 0으로 비교하거나 단순 포함 검사에 in을 사용한다.

포매팅은 표시 방식이다. f'{price:,}'로 쉼표를 넣어도 원래 price는 int다. 내·강사님 코드의 노래 문장 일부는 데이터 철자가 서로 다르므로 검색 위치를 비교할 때 동일 입력인지 확인해야 한다.
...
parts = clean.split()
print(repr(raw))
print(repr(clean), parts)
print(' / '.join(parts))
print(clean.find('사과'), clean.find('키위'))
price = 19000
print(f'{price:,}원', type(price).__name__)
```
...
1. raw를 직접 생성하고 strip으로 양끝을 정리한 결과만 clean에 연결한다.
2. clean.split()이 문자열 두 개를 포함한 새 list를 만든다.
3. 구분자 문자열의 join이 각 요소 사이에 ' / '를 넣어 새 str을 만든다.
4. 검색의 정수 반환값과 가격 포매팅을 출력한다. 원래 raw와 정수 price는 바뀌지 않는다.

### 원본에서 어디에 사용했을까?


#### 내 코드: `03_string.py` 83~89행
...
내·강사님 03번의 검색·split·join·strip을 데이터 정리 과정으로 연결했다. replace는 기본적으로 모든 일치를 바꾸지만 문자열 내부 숫자·상호명까지 무차별적으로 바꾸지 않도록 대상 필드를 먼저 분리한다.
...
1. clean.split(' ')의 결과는? 문자열에서 '원'을 제거한 결과는 숫자일까?
2. '1000원'.replace('원','') 뒤 어떤 변환이 있어야 500을 더할 수 있는가?

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. ['사과', '', '바나나']다. replace 결과는 여전히 str이며 계산하려면 int 등으로 변환한다.
2. replace 결과는 str '1000'이므로 int로 바꾸어 1500을 계산한다. 변환 실패 입력도 처리한다.
