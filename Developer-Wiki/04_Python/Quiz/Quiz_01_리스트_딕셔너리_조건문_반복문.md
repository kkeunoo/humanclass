...
- [학습 목표](#py-quiz-section-2)
- [개념에서 실제 실행까지 — Quiz 검토 — 정답 회차와 최다 득표자](#py-quiz-section-3)
- [11. Key Summary](#py-quiz-section-14)

<details>
<summary>상세 목차 전체 펼치기</summary>

- [문서 정보](#py-quiz-section-1)
- [학습 목표](#py-quiz-section-2)
- [개념에서 실제 실행까지 — Quiz 검토 — 정답 회차와 최다 득표자](#py-quiz-section-3)
- [1. 원본 문제 구성](#py-quiz-section-4)
- [2. 문제 1 - 짝수 리스트와 홀수 합](#py-quiz-section-5)
- [3. 문제 2 - 장바구니 전체 가격](#py-quiz-section-6)
- [4. 문제 3 - UP/DOWN 게임](#py-quiz-section-7)
- [5. 문제 4 - 로그인 검사](#py-quiz-section-8)
- [6. 문제 5 - 랜덤 투표 시스템](#py-quiz-section-9)
- [7. 원본 코드에서 확인할 부분](#py-quiz-section-10)
- [8. 종합 문제](#py-quiz-section-11)
- [9. 종합 문제 정답](#py-quiz-section-12)
- [10. Final Checklist](#py-quiz-section-13)
- [11. Key Summary](#py-quiz-section-14)
...
| 연결 문서 | `04_Python_리스트와_데이터처리.md`, `07_Python_딕셔너리와_집합.md`, `08_Python_조건문과_패턴매칭.md`, `09_Python_반복문.md` |
...
<a id="py-quiz-section-2"></a>

## 학습 목표

- 집계·시도 수·동점 요구사항을 검증한다.
- 예제의 입력·처리·출력과 대표 실패 조건을 직접 확인한다.

---

<a id="py-quiz-section-3"></a>

## 개념에서 실제 실행까지 — Quiz 검토 — 정답 회차와 최다 득표자

### 무엇이며 왜 배워야 할까?

문제 1의 짝수는 [10,22,8], 홀수 합은 38이고 문제 2 장바구니 총액은 25000이다. 품목별 단가×개수를 누적한 값이다.

UP/DOWN 원본은 틀린 회차만 count를 늘려 첫 정답이 0번으로 출력된다. 강사님 자료형 퀴즈는 비교 전에 증가시킨다. 다만 num=q3로 정답을 직접 넣는 시연 상태이므로 실제 입력 게임으로 완료했다고 설명하지 않는다.

max(votes)는 득표 수가 아니라 dict 키를 비교한다. 내 test.py와 강사님 자료형 퀴즈 모두 이 오류가 있다. 내 quiz/01_quiz.py의 a·b·c 카운트는 100표를 세지만 입력 후보명과 연결한 최다 득표자 출력은 미완성이다.

아래는 고정 투표로 get의 기본값 0에서 시작해 1씩 누적한다. max(votes.values())로 최다 득표를 구한 뒤 같은 득표의 모든 후보를 찾아 동점을 표시한다. 빈 투표에는 default=0과 빈 승자 목록을 사용한다.
...
print(votes)
print('키의 최댓값', max(votes))
highest = max(votes.values(), default=0)
winners = [name for name, count in votes.items() if count == highest]
print('최다 득표', winners, highest)
print('총 투표', sum(votes.values()))
```
...
1. 고정 ballots의 각 후보 이름을 순회한다.
2. 기존 득표가 없으면 get의 0을 사용하고 1을 더해 해당 키에 저장한다.
3. 키의 max와 값의 max를 따로 관찰하여 서로 다른 비교 대상임을 확인한다.
4. 최고 값과 같은 항목 모두를 winners로 모으고 전체 득표 합으로 누락 집계도 확인한다.

### 원본에서 어디에 사용했을까?


#### 내 코드: `01_quiz.py` 93~99행
...
강사님 퀴즈 폴더 이름은 인코딩이 깨져 ‘자료형 퀴즈(내용 기준)’로 표기한다. 로그인 평문 암호 예제는 분기 학습용이지 운영 인증 설계가 아니다.

### 실무에서 판단할 기준과 디버깅

### 강사님 별도 자료형 퀴즈와도 비교하기

| 항목 | 내 quiz/01_quiz.py | 강사님 자료형 퀴즈 |
| --- | --- | --- |
| 짝수 추출 | 인덱스 순회로 append | 직접 순회와 컴프리헨션 모두 제시 |
| 장바구니 | keys로 품목 이름 순회 | dict 직접 순회로 총액 누적 |
| UP/DOWN | 틀린 가지에서만 count 증가 | 비교 전 증가하되 정답을 num에 직접 넣은 시연 |
| 로그인 | 키 존재 확인 후 해당 값 비교 | 같은 기본 구조, get 방식도 추가 시연 |
| 투표 | a,b,c를 각각 증가, 최다 후보 미완성 | 숫자 변수·목록·dict 집계 여러 방식, dict 키 max 오류 남음 |

강사님 코드에서도 다음 부분은 득표 수 비교가 아니다. 원본 문맥 발췌이며 앞에서 득표 dict를 만든 상태를 전제로 한다.

```python
print(max(득표), 득표[max(득표)])
```

예를 들어 {'a': 2, 'b': 2, 'c': 1}에서는 c 1이 선택된다. 원본을 비교한다는 것은 어느 한쪽을 무조건 정답으로 삼는 것이 아니라 같은 입력에서 요구사항을 만족하는지 확인하는 일이다.
...
1. max(votes,key=votes.get)은 동점 후보 둘을 모두 반환할까?
2. 정답이 50이고 첫 입력이 50이면 시도 수가 0인 출력이 맞는가?

<details>
<summary>정답과 이유 보기 — 먼저 출력·상태를 예측한 뒤 펼치기</summary>

1. 아니다. 후보 하나만 반환한다. 같은 득표의 모든 항목을 별도로 수집해야 한다.
2. 시도 횟수 계약에서는 1이어야 한다. 유효 입력을 받은 뒤 비교 전에 증가시키되 잘못된 입력도 셀지는 정책을 명시한다.
